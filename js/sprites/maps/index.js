...
export const CLIMATE = {
  ...
  newworld: {
    ...
    /* ★ 회차 26 — 띠가 **둘뿐**이라 남미가 초록/모래 두 쪽으로 갈렸고, 경계가 파장 286px짜리
       사인 하나여서 화면에서는 **대륙을 가로지르는 곧은 금**으로 보였다(D5·검증가 B가 같이 지적).
       고유색도 17색으로 아홉 장 중 최소였다. ⇒ 가운데 톤(`mid` = 세하도)을 하나 넣어 셋으로
       가르고, 두 경계 모두 **긴 파(파장 300px)에 짧은 파(파장 110~130px)를 겹쳐** 흔든다. */
    zone: { forest: null, mid: '#6d9046', scrub: '#8a9a52', desert: '#c4a874' },
    sea: ['#13566e', '#1c6a84', '#082836'],
    shore: ['#e6d6ac', '#57a8b6', '#3892a6', '#256d86'],
    zones: { bands: [
      [(x) => 126 + Math.sin(x * 0.021 + 1) * 13 + Math.sin(x * 0.057) * 9, 'forest'],   // 아마존
      [(x) => 168 + Math.sin(x * 0.017 + 3) * 15 + Math.sin(x * 0.048 + 1) * 8, 'mid'],  // 세하도·차코
      [Infinity, 'scrub'],                                                               // 팜파스
    ] },
  },
  ...
  temperate: {
    ...
    /* ★★ 한반도만 띠에서 뺀다(회차 26). 위 띠는 **위도**만 보므로 y<29가 통째로 사막색이 되고,
       그러면 한반도의 북쪽 절반이 만주·화북과 **같은 모래색**이 되어 대륙에 녹아 붙는다.
       실제 창에서 「한반도가 안 보인다」의 절반은 지형이 아니라 이것이었다 — 반도가 y0~57인데
       그중 y0~29가 대륙과 한 색이니 눈에는 y29~57짜리 뭉툭한 혹만 남는다.
       고증으로도 이쪽이 맞다: 만주·화북은 마른 스텝이지만 **한반도는 남북이 다 산림**이다.
       상자는 반도만 감싼다(x150~226 · y5~64) — 울릉도(238,18)·쓰시마(228,67)·제주(190,68)는 밖이다.
       y>5로 잘라 만주와 이어지는 맨 위 5px은 대륙색으로 남긴다(가로 이음매가 화면 밖 가장자리에 숨는다). */
    zones: {
      extra: (x, y) => {
        if (y >= 64 || x >= 226) return null;                       // 울릉도(238,18)·쓰시마(228,67)는 밖
        // 만주와 가르는 선 — 압록강~두만강. **직선이면 자 자국**이라 흔들고, y16부터는 x150에 묶는다
        // (안 묶으면 선이 남쪽으로 계속 밀려 반도 서안을 되먹는다)
        if (x <= Math.min(150, 134 + y * 1.1) + Math.sin(y * 0.3) * 3) return null;
        return y < 30 + Math.sin(x * 0.07 + 2) * 6 ? 'forest' : 'mid';   // 북쪽은 산림, 남쪽은 온대
      },
      bands: [
