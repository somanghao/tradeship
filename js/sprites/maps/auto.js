...
export function coastOctaves(land, opts = {}) {
  ...
  /* ★★★ 회차 26 실측 — **바다를 먹는 것은 「좁은 물길이 막히는 것」이 아니라 「해안선의 비대칭」이었다.**
     깎기는 `(thick-3)*0.28`(대륙에서 최대 2.7px)로 막혀 있는데 붙이기는 문턱(8px) 위에서
     **상한이 없어 14.1px까지** 갔다. 노이즈는 평균 0인데 음의 절반만 잘리니 해안이 늘
     바다 쪽으로 1.5~2px씩 부푼다 — 지중해 다각형 36%가 렌더 32~33%가 되는 3~4점이 그것이다.
     (가드 문턱을 8→14로 올리고 반경을 17로 키워 봤으나 바다%는 오히려 -1점이었다.
      최대필터 반경을 키우면 좁은 물길이 옆 넓은 바다의 값을 **새로 받아** 가드가 도리어 풀린다.)
     ⇒ 문턱을 버리고 **붙이기에 늘 비율 상한**을 건다 — *어떤 물길도 제 폭의 40%보다 많이 먹히지 않는다.*
       넓은 바다는 상한이 커서(반폭 12px → 4px) 예전처럼 굽이치고, 좁은 물길만 지켜진다.
       아드리아해 최협부가 12~13px → 15~16px이 됐다.
     ⚠️ **깎기까지 같은 비율로 만들어 완전 대칭으로 하면 안 된다** — 그러면 바다는 +2점인데
       **섬 항구(6px 안에 뭍 0)가 아홉 장 전부에서 는다**(지중해 2→4 · 중동 0→3 · 인도양 2→5 ·
       동남아 0→4 · 카리브 0→3 · 남미 0→4). 해안을 깎으면 **항구가 딛고 선 뭍이 깎인다.**
       회차 26이 실제로 해 보고 되돌린 자리다. */
  ...
  for (let y = 0; y < VH; y++) {
    for (let x = 0; x < VW; x++) {
      ...
      if (push > 0) push = Math.min(push, Math.max(0, seaThick[i] - 2) * 0.4);
      out[i] = sdf < push ? 1 : 0;
    }
  }
  return out;
