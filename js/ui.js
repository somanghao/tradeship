...
/* ── 글 ─────────────────────────────────────────────────────────
   ...
   ★ 왜 여기 두나. `이(가)`·`을(를)`는 이름이 무엇이든 상관없이 찍어 내는 표기이고,
     그 순간 문장은 사람이 쓴 것이 아니라 코드가 뱉은 것이 된다. 이 게임은 한 줄에
     인물이 서는 것을 노린 물건이라 그 자국이 유난히 크게 남는다
     ("오루치 레이스호이(가) 바람을 타고 다가온다"를 실제로 만났다).
     받침을 보고 고르면 되는 일이고, 씬 넷이 같은 규칙을 써야 하므로 헬퍼로 둔다. */
