...
/** 바닥까지 내려갔을 때 남는 배 — **그 바다의 삭은 배**다.
    ★ 시작배는 2026-08-26에 아홉으로 갈렸는데 **청산·침몰 뒤에 남는 배는 `hulk` 하나**였다.
      광저우에서 파산한 사람이 지중해 배를 받는 셈이라, 같은 규칙을 여기도 세운다.
      제원은 아홉이 나란하므로(±20% 안) **난이도는 안 움직이고 얼굴만 맞는다.** */
export const wreckShipOf = (regionId) =>
  START_PORTS.find((p) => p.region === regionId)?.ship ?? BANKRUPT.keepShip;
...
export const HOLDINGS = {
  ...
  dock:     { name: '부두',      store: 0,  priceBase: 60000, priceByIndustry: 40000,
              ...
              /* ★ 예전 설명에 **「공업력」이라는 말이 없었다**(C-18 ⓐ). 이 거점이 올리는 것이
                 조선소 화면의 그 숫자라는 것을 플레이어가 알 방법이 없었다. */
              desc: '부두를 늘린다 — 이 항구의 공업력이 1 오르고, 그만큼 큰 배를 짓는다.' },
...
export const HULL = {
  ...
  /* ★ **침몰(C-13 N4).** 오래 「고를 자리」를 못 찾던 규칙이다. 폭풍(항차당 12%)에 붙이면
     금고 0·선체 1에서 1일 항로를 열여섯 번 돌아 빠져나온 실플레이가 **87% 확률로 끊긴다** —
     그것은 위험이 아니라 사고다. 그래서 **우연이 아니라 플레이어가 고른 자리**에 붙인다:
     **삭은 배로 전투에 져야** 가라앉는다. 피하는 길이 셋 다 열려 있다 —
     ① 수리한다 ② 도망친다(`fleeOdds`) ③ 애초에 안 싸운다.
     ⇒ 「삭은 배를 수리 없이 무한히 몬다」의 대가가 처음으로 실재하되, **막지는 않는다.** */
  sinkAt: 0.10,     // 전투에 지는 순간 선체가 이 비율 밑이면 기함이 가라앉는다 (= crawlAt과 같은 눈금)
};
...
export const CHAIN = [
  ...
  /* ── 2단계 · 사슬 넷이 더 붙는다 (SPEC-vertical §1-3) ─────────────────────
     1단계는 직물 셋이었다. 여기서 **설탕 · 비단 · 은 · 후추 · 도자기**가 붙어 일곱이 된다.
     ★ 정수는 감이 아니라 **밴드에서 역산**한 값이다(1.08~1.25 · `check-chain`이 실패시킨다). */
  {
    id: 'refine_sugar', name: '제당', work: '제당소',
    in: { cane: 7 }, out: { sugar: 1 }, req: 1,
    // 280 ÷ (34×7) = 280/238 = **1.176**
    // ★ 7:1이 이 사슬의 전부다 — 즙을 졸이면 그만큼 준다. 그리고 벤 줄기는 며칠을 못 견뎌
    //   **밭 옆에서 졸여야 한다**(엔제뉴). 그래서 산지와 제당소가 같은 항구에 서고
    //   팔 데가 북유럽이라, 이 사슬의 대가는 돈이 아니라 **항로**가 된다.
    blurb: '벤 줄기를 짜서 즙을 내고 가마에 졸여 굳힌다. 밭에서 며칠을 못 넘긴다.',
  },
  {
    id: 'weave_silk', name: '제사·직조', work: '직조장(견)',
    in: { rawsilk: 2 }, out: { silk: 1 }, req: 2,
    // 420 ÷ (190×2) = 420/380 = **1.105** — 밴드 하단이다.
    // 견직은 손이 많이 가는 대신 남는 것이 얇았다(생사값이 직물값의 40~55%였다).
    // 공업력 2를 요구하는 것은 견직이 도시의 기술이었기 때문이다 — 루카·제노바·베네치아.
    blurb: '고치에서 실을 켜고 베틀에 걸어 비단을 짠다. 도시의 기술이다.',
  },
  {
    id: 'smelt_silver', name: '제련', work: '정련소',
    in: { silverore: 15, mercury: 1 }, out: { silver: 2 }, req: 1,
    // 480×2 ÷ (31×15 + 310) = 960/775 = **1.239**
    // ★ **공업력 1인 유일한 이유가 지도다.** 포토시도 우앙카벨리카도 `industry 0`인 내륙
    //   광산이라 2를 걸면 이 사슬이 통째로 죽는다. 정련은 실제로 광산 옆에서 했다.
    // ★ 그리고 **수은이 없으면 안 돈다** — 아말감법이다. 수은 산지(우앙카벨리카·말라가)를
    //   물려야 하므로 사슬 하나에 산지가 둘이다.
    blurb: '으깬 광석에 수은을 섞어 은을 붙여 낸 뒤 불로 수은을 날린다.',
  },
  {
    id: 'dry_pepper', name: '건조·선별', work: '정제장',
    in: { rawpepper: 5 }, out: { pepper: 3 }, req: 1,
    // 230×3 ÷ (115×5) = 690/575 = **1.200**
    // 데쳐 볕에 말리면 무게가 절반 아래로 준다. 5:3이 그 방향이다.
    blurb: '딴 열매를 데쳐 볕에 말리고 고른다. 마르면 무게가 절반으로 준다.',
  },
  {
    id: 'kiln_ceramic', name: '요업', work: '요장',
    in: { kaolin: 4 }, out: { ceramic: 1 }, req: 2,
    // 130 ÷ (28×4) = 130/112 = **1.161**
    // 유럽이 못 만들던 것. 흙은 싸고 굽는 기술이 값이라, 공업력 2가 이 사슬의 문지기다.
    blurb: '백토와 백돈자를 이겨 굽는다. 유럽이 삼백 년을 못 흉내 낸 배합이다.',
  },
];
...
export const WORKS = {
  ...
  /* ── 2단계 · 생산 (SPEC-vertical §2-4·2-5) ────────────────────────────
     ★ **여기서 처음 곡선이 움직인다.** 1단계(가공장만)는 "거의 안 남는 것이 정상"이었고
       (`check-chain`의 대조식이 1.15~1.22로 그것을 말한다), 남기 시작하는 것은
       **밭이 원료를 원가로 댈 때**다. 그 원가분은 `impact`를 안 쌓는다 — 자기 밭에서 실었다.
     ★ 값 공식은 농장·광산이 같다: `(3,000 + base×50) × (0.7 + 0.15×size)`.
       그래서 **비싼 작물의 밭이 비싸다** — 정향 농장 37,000닢은 대형 갈레온과 같은 자리다.
     ★ 광산이 농장의 절반만 캐는 이유 — 광물은 값이 높아 **같은 칸이 훨씬 큰 이득**이다. */
  farm: {
    name: '농장', priceBase: 3000, priceByBase: 50, kind: 'crop', perPort: 3,
    perDay: { 1: 4, 2: 7, 3: 10 },      // 하루에 밭에 쌓이는 칸
    stockDays: 45,                       // 재고 상한 = 하루 산출 × 이 일수 (넘으면 썩는다)
    desc: '그 항구에서 나는 작물을 원가로 얻는 자리. 창고가 먼저다.',
  },
  mine: {
    name: '광산', priceBase: 3000, priceByBase: 50, kind: 'mineral', perPort: 3,
    perDay: { 1: 2, 2: 4, 3: 6 },
    stockDays: 45,
    desc: '그 항구에서 나는 광물을 원가로 얻는 자리. 창고가 먼저다.',
  },
  /* ── 3단계 · 소매 (SPEC-vertical §2-5·3-1) ───────────────────────────
     ★ **판매소가 파는 것은 마진이 아니라 「시장 깊이」다.** 320칸짜리 배로 한 항구에
       쏟으면 `MARKET.cap 0.50`에 닿는데, 판매소는 그 벌점을 **시간으로 바꾼다.**
       값을 시간으로 사는 것이고, 그래서 새 수입원이 아니다.
     ★ **팔 때만 감쇠한다.** 살 때도 깎으면 같은 항구에서 사고팔기를 되풀이하는
       무한 루프가 열린다 — 소매는 파는 자리다. */
  shop: {
    name: '판매소', priceBase: 2000, priceByBase: 22, perPort: 3, bySize: true,
    desc: '그 항구에서 천천히 흘려 판다. 쏟아붓는 벌점을 시간으로 바꾼다.',
  },
};
...
export const WORK = {
  ...
  /* ── 2단계 · 밭이 대는 원가 (SPEC-vertical §2-5) ────────────────────────
     그 항구에서 그 품목을 살 때 **밭 재고까지는 이만큼 싸게** 산다. 그리고 그 몫은
     `addPressure`를 하지 않는다 — 시장에서 산 것이 아니라 **자기 밭에서 실은 것**이다.
     ⚠️ `costFor`가 두 구간을 갖게 되므로 `buy()`의 이분 탐색이 전제하는 **단조 증가**가
       깨지지 않는지 검사에 넣었다(원가 < 시세이므로 단조는 유지된다 · `test-rules`). */
  farmOff: { 1: 0.12, 2: 0.18, 3: 0.24 },
  farmPriceBySize: { base: 0.7, per: 0.15 },   // (0.7 + 0.15×size)

  /* ── 3단계 · 판매소 (SPEC-vertical §2-5) ──────────────────────────────
     `shopCut` — 그 항구에서 **팔 때** 시장에 쌓이는 압력을 이만큼 덜어 준다.
     `shopFlow` — 위탁해 둔 짐이 하루에 이만큼 팔린다(그날 시세로).
     `shopFee` — 위탁 판매 수수료. 관세는 그것대로 따로 문다. */
  shopCut: { 1: 0.35, 2: 0.50, 3: 0.65 },
  shopFlow: { 1: 3, 2: 5, 3: 8 },
  shopFee: 0.12,
};

/* ── 3단계 · 위탁 — 남의 배에 실어 보낸다 (SPEC-vertical §3-3) ──────────────
   ★ **초반의 문**이다. 정기선은 동행선을 하나 빼야 하므로 선단이 자란 뒤에나 성립하는데,
     위탁은 **창고 둘**만 있으면 된다. 대신 수수료 8~15%가 항차 ROI 중앙값(10.1%)과
     맞먹어 *"급할 때만 쓰는 길"*이 된다.
   ★ **짐만 옮긴다 — 사고팔지 않는다**(§3-1의 경계). 도착하면 그 항구 **창고**에 들어간다. */
export const CONSIGN = {
  feeBase: 0.06,      // 수수료 = 시가 × (이것 + 그 항로 요율/100)
  daysMul: 1.4,       // 남의 배는 제 일정으로 다닌다
  lossMul: 0.5,       // 유실 확률 = 항로 요율(%) × 이것
  capPer: 40,         // 한 건에 실을 수 있는 칸
  maxOpen: 5,         // 동시에 띄울 수 있는 건수
  insureMul: 2,       // 보험을 켜면 수수료가 이만큼 는다
  cover: 0.30,        // 유실 시 시가의 이만큼을 받는다
};

/* ── 3단계 · 정기선 — 동행선 하나를 항로에 묶는다 (SPEC-vertical §3-2) ────────
   ★ **값은 돈이 아니라 선단이다.** 묶은 배는 화물칸(`cargoCapTotal`)·포화력(`consortGuns`)·
     피해 분산(`shieldPer`)에서 빠진다. 그것이 이 장치의 대가다.
   ⚠️ **`tickLines(n)`은 `advanceDays()`와 `waitDays()` 양쪽에서 불러야 한다.**
     한쪽만 걸면 *"항구에 서 있는 동안 정기선이 멈춘다"*(또는 그 반대)가 된다. */
export const LINE = {
  max: 3,             // 동시에 굴릴 수 있는 정기선
  holdRate: 0.70,     // 그 배 화물칸의 이만큼만 싣는다(최소 인원이라 다 못 싣는다)
  turnPad: 4,         // 왕복 주기 = voyageDays×2 + 이것(하역 이틀씩)
  goodsMax: 3,        // 나르는 품목 수
  lossShip: 0.15,     // 짐을 털렸을 때 배까지 잃을 확률
};
