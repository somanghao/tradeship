...
/* ── 상단(商團) — 자본이 물가를 누른다 ────────────────────────────────
   ★ **`MARKET.impact`로는 물가가 절대 안정화되지 않는다.** 저것은 *부호 없는* 스칼라라
     사도 늘고 팔아도 늘고, 결과는 언제나 「거래를 불리하게」다(위 `pressureOf` 주석 그대로).
     NPC가 아무리 많이 날라도 **싼 항구는 계속 싸고 비싼 항구는 계속 비싸다.**
   ⇒ 그래서 **부호 있는 항**을 따로 판다. 상단이 싼 데서 사가면(flow<0) 그 항구 값이 오르고,
     비싼 데로 부으면(flow>0) 내린다 — **양끝이 서로에게 다가간다.** 그것이 실제 상단이 한 일이다.
     `guildFactor`가 `shockFactor` 옆에서 곱해진다(`state.js: priceOf`).

   ⚠️ **「어렵게」와 「불가능」은 다르다.** 평탄화하면 무역 게임의 몸통이 사라지므로
     남길 셋을 **규칙으로** 남긴다 — ① 위험 항로 프리미엄(`riskAversion`)
     ② 사건의 큰 기회(`reactDays`) ③ 상단이 안 건드리는 틈새(`minMargin`·`minLot`).
   설계 정본 → `.playtest/round-25/NPC-DESIGN.md` · 관측 → `node tools/sim-guild.mjs 20` */
export const GUILD = {
  enabled: true,       // ★ 짝지은 측정(`sim-guild.mjs`)이 이것을 껐다 켠다
  hops: 3,             // 몇 칸 건너까지 보나. **지금 NPC는 1칸이라 먼 가격차를 못 먹는다**
  minMargin: 0.10,     // 이 마진률 미만이면 안 뛴다 → 잔 차익은 플레이어의 몫
  minLot: 12,          // 이 물량 미만이면 안 뛴다 → 작은 배의 자리
  /* 길 위 요율의 **합** 1%p당 점수 감점. ★ 처음엔 `routeRisk(상관, 목적지)` 하나로 쟀는데
     두세 칸 건너는 직항이 아니라 기본값이 나와 **위험 항로도 똑같이 눌렸다**(실측이 잡았다).
     지금은 길 위의 요율을 더해서 잰다 → 위험한 길은 상단이 안 가고 그 프리미엄이 플레이어에게 남는다. */
  riskAversion: 0.030,
  reactDays: 6,        // 사건이 난 지 이 날 수 안에는 손대지 않는다 → 큰 기회는 플레이어의 것
  /* ★★ **0.85 → 0.055 (실측이 설계를 뒤집었다).** 0.85에서는 **한 항차만으로 상한을 4배 넘겨**
     손댄 자리가 전부 ±상한에 눌어붙었다 — 곧 상단이 「값을 좁히는 장치」가 아니라
     **±14% 난수 발생기**가 됐다. 인접 항구의 실제 가격차 중앙값이 **5.4%**인데
     그 위에 ±14%를 얹었으니 당연하다: 짝지은 실측에서 가격차가 **+21.3%**(손댄 항로는 +63%),
     주인공 금고는 **−49~−60%**. 「좁힌다」의 정반대였고 「어렵게」도 아니라 「불가능」이었다.
   ⇒ 값을 미는 힘은 **비례**여야 한다 — 한 항차가 몇 %를 밀고, 균형은 상한이 아니라
     **`minMargin`(더는 남는 게 없어 안 뛴다)**이 잡아야 한다. 그것이 실제 차익거래의 모양이다. */
  flowK: 0.055,        // flow 한 단위가 값을 미는 세기 (÷ marketDepth)
  priceCap: 0.10,      // ★ 한 항구·품목이 상단 때문에 움직일 수 있는 최대 폭(±) — 안전망이지 목표가 아니다
  flowDecay: 0.975,    // 날마다 남는 비율 (반감기 ≈ 27일) — 상단이 손을 놓으면 값이 되돌아온다
  loadRatio: [0.40, 0.80],
  holdBase: 90,        // 한 항차의 칸 — rank가 붙는다 (rank5 ≈ 130칸)
  convoySpeed: 1.0,    // 선단 속력 (일수 = 거리 ÷ (13 × 이것))
  reachCap: 16,        // 상관 하나가 보는 후보 항구 수 상한 — 시야를 늘려도 시뮬이 끝나게
  goodsCap: 14,        // 상관 하나가 보는 후보 품목 수 상한
  /* 전문 품목이 아닌 것의 점수 배율. ★ 1.0으로 두면 상단이 아무거나 나르는 잡상이 되고,
     0(전문만)으로 두면 **세계의 얇은 조각만 만져 주인공의 장부가 한 톨도 안 움직인다**
     (20시드 실측: 10항차 +0.0% · ROI +8.14%p — 되레 쉬워졌다). 그 사이가 이 값이다. */
  offSpecialty: 0.72,
  dayCost: 90,         // 선단 하루 운영비(닢)
  /* ★ **싣고 부리는 데 걸리는 날.** 이것이 없으면 점수(=이익÷일수)가 **한 칸짜리 뜀뛰기**를
     압도해 상단이 180일에 2만 항차를 뛴다 — 「무역로를 따라」가 아니라 제자리 뜀뛰기다.
     사료의 상단은 정기시·적하·검량에 열흘 안팎을 썼고, 그래서 **먼 항로가 값어치가 있었다**.
     이 한 줄이 상단을 **장거리 쪽으로** 돌려세운다. */
  portDays: 10,
  flowCapMul: 1.2,     // ⚠️ **쌓이는 flow에도 상한을 둔다** — 안 두면 상한에 붙어 값이 안 돌아온다
  /* 사료 자본(닢 환산 · `content/houses-evidence.json`)을 **게임 자본**으로 누른다.
     ⚠️ 그대로 쓰면 푸거와 지방 상단이 1000배 차라 세계가 한 상단의 것이 된다.
     서열은 남기고 크기만 누르는 것이 목적이다: cap0 = capBase × (사료 ÷ capRef)^capPow */
  capBase: 40_000, capRef: 1_000_000, capPow: 0.42,
  fleetStep: 30_000, fleetCap: 6,     // 자본 → 선단 (물가를 누르는 힘)
  mightStep: 120_000,                 // 자본 → 압박 함대의 세기 1~5 (`strength`와 같은 눈금)
  upkeep: 0.0006,      // 자본에 붙는 하루 유지비 — 안 굴리면 마른다(무한 성장 방지)
  /* ★★ **상관은 공짜가 아니다 — 이 한 줄이 파산을 성립시킨다**(회차 26).
     자본 유지비(`upkeep`)는 자본에 **비례**하므로 아무도 못 무너뜨린다: 자본이 줄면 유지비도 준다.
     그리고 `minMargin`이 10%를 보장하는 한 **장사가 되는 상단은 절대 안 망한다.**
     ⇒ 실측: 항차 손실만으로는 1,440일에 문 닫은 곳이 **0**이었고 총자산이 장부가의 91% 아래로
       내려간 상단조차 없었다. 무너지는 길이 규칙에 없었던 것이다.
     ⇒ 사료가 그 답을 갖고 있었다 — **팩토리(상관)는 건물·직원·현지 관리에 정액이 든다.**
       수지가 안 맞는 상관은 닫았고, 그것이 상사들이 실제로 무너진 자리다.
     ⇒ 정액이므로 **장사를 못 하는 상단이 실제로 마른다**. 덤으로 무한 합병의 브레이크가 된다
       (상관을 많이 삼킬수록 무겁다). */
  seatUpkeep: 150,     // 상관 하나당 하루 유지비(닢 · 정액) — 300이면 1,440일에 스물넷이 무너졌다(너무 많다)
  /* 세 갈래 상호작용 */
  coolDays: 45,        // 한 상단이 다시 걸기까지
  /* ★ −20이면 새 판에 괴롭히는 상단이 **둘뿐**이었다(temper 분포 중앙값이 0.35라 낮은 쪽으로 쏠려 있다).
     여섯쯤은 처음부터 사나워야 "부가 힘이 된다"가 초반에 보인다 — 나머지는 악명이 만든다. */
  pressAt: -12,        // regard 이하면 괴롭힌다
  helpAt: 25,          // regard 이상이면 돕는다
  /* ★ 상단의 성격이 곧 첫인상이다 — 이것이 없으면 새 판의 호감이 전부 0이라
     **세 갈래 중 「이용」 하나만 돈다**(규칙은 멀쩡한데 아무 일이 안 나는 자리). */
  temperRegard: 90,
  regardPerInfamy: 6,  // 악명 1당 상단 호감 −6 — 그 깃발 배를 털면 그 상단이 돌아선다
  cornerLot: 30,       // 매점·투매 한 번의 물량
  creditOff: 0.06,     // 도움: 그 상단 seats에서의 매입 할인
  escortOff: 0.10,     // 도움: 그 구간 조우 확률 상대 감소
  useFee: [0.05, 0.12],// 이용(사주): 보수 = 그 상단 자본의 이 비율
  actBase: 0.0016,     // 하루 판정 = 이것 × sway(선단×세기). 자본이 클수록 자주 건다
  pressDays: 30,       // 압박 함대가 그 자리에 서 있는 날
  /* 압박 함대가 타는 배 — 세기 1~5. ★ `pirateEnemy`가 이 배로 hp·포를 낸다.
     ⚠️ 상단은 해적이 아니라 **호위 선단**이라 화물선 계열을 섞는다(플뤼트·캐랙). */
  escortShips: ['fluyt', 'brig', 'carrack', 'frigate', 'galleon'],
  escortPurse: 0.004,  // 갑판에 실린 금화 = 자본의 이 비율 (`capLoot`이 다시 누른다)
  regardOnSlain: -25,  // 그 상단의 호위선단을 꺾으면 호감이 이만큼 더 내려간다
  helpDays: 60,        // 신용장·호위가 사는 날
  offerDays: 45,       // 사주의 기한
  regardUse: 18,       // 사주를 마치면 의뢰한 쪽 +, 당한 쪽 −

  /* ══ 흥망 — 인수·합병·파산 (회차 26) ═══════════════════════════
     설계 정본 `.playtest/round-26/C-DESIGN.md` · 관측 `node tools/probe-houses.mjs`

     ★ **왜 필요했나(실측)**: 손대기 전 세계를 두 해 굴리면 **49곳이 전부 선단 6·세기 5**가 됐다.
       사다리가 천장에 눌어붙어 사라지고, 시작 자본의 17.4배 서열이 4.6배 안으로 모인다.
       평형 자본이 `cap0`가 아니라 **`rank`가 정한 `holdBase`**에 묶여 있기 때문이다 —
       자본이 커져도 한 항차의 칸이 안 커지는데 유지비만 자본에 비례하므로 한 점으로 모인다.
       그리고 손실 항차가 없어 **아무도 안 망한다**. 「서로 먹고 먹힌다」가 설 자리가 없었다. */

  /* ── ① 결산과 배당 — 자본은 무한히 안 쌓인다 ────────────────
     사료: 상사는 2~3년마다 결산(Rechnung)하고 이익을 조합원에게 나눴다. 자본을 영원히
     쌓아 둔 상사는 없다. ⇒ `cap0 × capKeep`을 넘는 몫의 `payout`만큼이 **세계 밖으로** 나간다.
     ⚠️ 배당은 플레이어에게 **안 온다** — 상단의 돈이 주인공 주머니로 새는 문을 만들지 않는다
       (자동 매매 창구를 안 만든다는 규약과 같은 선). */
  bookDays: 365,       // 결산 주기(일) — 사료의 연차 결산(이탈리아·한자 상사가 해마다 장부를 닫았다)
  capKeep: 8,          // 결산 뒤 남기는 자본 = cap0 × 이것
  payout: 0.80,        // 그 위를 이 비율만큼 배당한다 (0.60·2년 결산으로는 자본이 cap0의 32배에서 놀았다 — 실측)

  /* ── ② 항차가 바다에서 사라진다 ─────────────────────────────
     ★ **요율이 곧 손실률이다**(사료). 이 저장소는 `ROUTE_RISK`를 *당대 인수업자가 매긴 요율*로
       적어 두고 `insuranceFor`가 그대로 쓰는데, **상단의 항차에는 한 번도 안 물렸다.**
     ⚠️ 플레이어의 조우 확률은 한 톨도 안 바뀐다 — 상단의 항차는 지도에 배로 안 뜬다. */
  /* ★ **요율 1%p = 손실 확률 1%.** 그것이 보험요율이라는 말의 뜻이다 — 임의의 계수가 아니다.
     (0.004로 시작했다가 사료 쪽으로 올렸다. 길 위 요율의 **합**을 쓰므로 세 칸짜리 항로는
      그만큼 위험하다 — 그것도 사료다.) */
  lossPerPct: 0.010,   // 길 위 요율 합 1%p당 그 항차가 안 닿을 확률
  /* ★ **상단은 적하보험을 들었다**(사료 — 이 게임에도 `INSURANCE_COVER`가 있다).
     보험 없이 매입액을 통째로 날리게 두면 **한 번 잃은 상단이 그대로 죽는다**(실측:
     120일에 넷이 무너졌다 — 자본이 20%로 떨어지면 싣는 양이 줄고 이익이 줄어 되돌아오지 못한다).
     그것은 흥망이 아니라 **한 번의 주사위**다. ⇒ 매입액의 이만큼은 돌려받는다. */
  lossCover: 0.55,
  /* ⚠️ **안 닿은 짐의 사건은 제 상수를 두지 않는다** — `SHOCK.raidMult`/`raidDays`를 그대로 쓰고
     **그 항구가 실제로 사들이던 물건일 때만** 건다(`world.js: raids()`와 완전히 같은 규약).
     제 상수(1.35·40일)를 두고 아무 항구에나 걸었더니 사건 폭 지표가 **−36.4%**로 떨어졌다 —
     상단의 항차는 하루에 한 척꼴로 안 돌아오므로 **작은 사건을 뿌려 큰 기회를 묽게 만든다.**
     사건 밀도(`SHOCK.densityBase`)는 맞춰 둔 값이다. 눈금 없는 사건원을 얹지 말 것.
     ⇒ `demand` 문턱만으로도 활성 사건이 6 → 24건으로 **네 배**가 됐다. 그래서 문턱을 하나 더 둔다 —
       **잃은 양이 그 항구의 시장 깊이에 견줘 커야** 한다(`lossShockShare`). 큰 항구는 한 배로
       안 흔들리고 작은 항구는 흔들린다. 새 눈금이 아니라 이미 있는 `marketDepth`를 쓴다. */
  lossShockShare: 1.2,

  /* ── ③ 부실 → 파산 → **인수** ──────────────────────────────
     ★ **상관이 빈 항구를 만들지 않는다.** 회차 25가 이 기능을 미룬 이유가 정확히 그것이었다.
       파산은 소멸이 아니라 인수다 — 명부(`js/npc/houses*.js`)는 한 줄도 안 지운다. */
  /* ★ **장부가(제 최고점)에 견준다 — 사료 자본이 아니다.** `cap0`로 재려 했더니 판이 돌면
     자본이 그 10~28배에서 놀아 **문턱에 영영 안 닿았다**(실측: 2,880일에 문 닫은 곳 0).
     장부가는 결산 때 다시 열린다(`g.peak`) — 「배당해서 줄었다」를 추락으로 읽지 않기 위해서다. */
  bustAt: 0.50,        // 총자산 < 장부가 × 이것이면 부실
  bustDays: 180,       // 부실이 이만큼 이어지면 문을 닫는다 — 반년을 장부가의 절반 아래로 못 버티면
  seatPrice: 12_000,   // 인수가 = 남은 자본 + 상관 수 × 이것 (인수는 공짜가 아니다)

  /* ── ④ 합병 — 이것이 서열을 실제로 바꾼다 ───────────────────── */
  mergeCheckDays: 180, // ⚠️ 90일마다 보면 판이 도는 동안 바다가 계속 줄어든다 — 반년에 한 번
  mergeRatio: 5,       // 총자산이 이 배 넘게 크고 상관이 겹치면 산다
  /* ★ **작기만 해서는 안 되고 기울고 있어야 산다.** 이 줄이 없으면 첫 판정에서 일곱 곳이
     한꺼번에 사라졌다 — 흥망이 아니라 한 번의 정리해고다. 사료의 인수도 「작은 경쟁사」가
     아니라 「무너지는 경쟁사」를 샀다. 총자산이 **제 장부가**의 이 배 아래일 때만 먹잇감이 된다. */
  mergeWeakAt: 0.65,
  mergePrem: 1.30,     // 합병가 = 작은 쪽 총자산 × 이것 — 치르므로 직후에는 약해진다
  minHouses: 3,        // ⚠️ 한 바다에 남겨 두는 살아 있는 상단의 하한 (세계가 한 상단의 것이 되면 안 된다)

  /* ── ⑤ 재기 — 콘텐츠 보존 장치 ──────────────────────────────
     사료: 무너진 상사의 이름은 후계 상사로 다시 섰다(벨저 → 후손 상사 · 메디치 지점). */
  reviveDays: 360,     // 문 닫은 뒤 이만큼 지나야 다시 설 수 있다(상단의 시계로 센다)
  reviveCap: 0.5,      // 다시 설 때의 자본 = cap0 × 이것
};
...
export const OFFICER = {
  ...
  /* 성과급 — 능력(아래 perks)·급여와 **한 묶음으로** 맞춘 값이라 하나만 건드리면 균형이 깨진다.
     ...
     ★ 자동 동행으로 바꾸면서 전제가 무너져 **재측정했다.** 위 표는 "중반에 계약금 1,800을
       ...
       **가난할 때 급여를 미루는 규칙**을 넣는 편이 서사와 맞는다
       → ✅ 아래 `defer`가 그것이다(2026-08-30 · 회차 27). */
  cut: 0.11,

  /* ── 급여 유예 (2026-08-30 · 회차 27) ────────────────────────
     **가난할 때 에이미의 삯만 미룬다.** 위 주석이 "wage를 낮추지 말고 미루는 규칙을
     넣으라"고 적어 둔 자리이고, 낮추는 것과 미루는 것은 결과가 전혀 다르다 —
     낮추면 **모든 판이** 쉬워지고, 미루면 **미룬 판만** 다른 값을 문다.

     ── 무엇을 얻나 ────────────────────────────────────────────
     급여일의 벌은 **못 준 비율**(`missed / owed`)로 매겨진다(`state.js: settlePayroll`).
     그런데 그 `owed`에 에이미 몫이 섞여 있어서, **떠날 수 없는 사람에게 못 준 돈이
     떠날 수 있는 사람의 불만으로 계산되고 있었다.** 유예는 그 몫을 분모에서 뺀다.
       예) 금고 100 · 선원 216 · 에이미 78 ⇒ 못 준 비율 0.66 → **0.54**
           금고 216이면 0.27 → **0.00** (이탈 판정 자체가 안 열린다)
     ⇒ 얻는 것은 돈이 아니라 **갑판이 흩어지지 않는 것**이다.

     ── 무엇을 무나 (★★ 공짜 유예는 초반 난이도를 통째로 깎는다) ──
       ⓐ `rate` 미룬 삯이 달마다 **×1.15**로 분다. 무이자로 미루면 언제나 미루는 것이 옳다.
       ⓑ `share` **한 달 미룰 때마다 에이미의 성과급이 영구히 +0.5%p** 오른다(상한 +1%p = 평생 두 번).
          ★★ **이것이 이 규칙의 몸통이고, 처음 짠 값은 틀렸다.** 처음에는 「미룬 삯이 남아
            있는 동안만」 성과급을 올렸는데, 실측에서 이탈이 **28/40판 → 4/40판**으로 사라지고
            그 대가가 에이미 몫 +36%(213닢)뿐이었다 — 잃을 뻔한 짐이 2,796닢이었으니
            **미루지 않을 이유가 없는 단추**였다. 그것은 선택지가 아니라 난이도 인하다.
          ⇒ 대가를 **영구 지분**으로 바꿨다. 미루는 순간의 값은 여전히 싸지만
            (가난할 때는 성과급을 뗄 이익 자체가 없다) **후반 90항차에 +1%p는 약 1,580닢**이다.
            초반에 얻고 후반에 문다 — **동료의 코멘다와 정확히 같은 모양**이고
            (`COMMENDA`: 초반엔 자본, 후반엔 비용) 이 저장소가 이미 옳다고 판정한 형태다.
          ★ 서사도 그쪽이 맞다. 에이미는 하인이 아니라 **동업자**다 —
            동업자에게 삯을 못 주면 그 사람의 **지분**이 커지는 것이 이 관계의 자연스러운 결말이고,
            그래서 이 규칙에는 「갚으면 원래대로」가 없다. 준 지분은 돌아오지 않는다.
       ⓒ **평생 두 번까지다** — 문지기는 `shareMax`이지 횟수가 아니다(`state.js: canDeferOfficer`).
          ★ 처음에는 「연속 석 달」로 뒀는데, 연속을 세면 **한 달 걸러 미루면 영영 미룰 수 있다**
            (실측에서 여섯 달 중 3.88달을 미뤘다). 지분이 상한에 닿은 뒤로는 이자만 물면 되니
            그때부터는 사실상 공짜였다 — **횟수 제한은 제한이 아니었다.**
          ⇒ 내줄 지분이 남아 있는 동안만 미룰 수 있다. 끝없이 미룰 수 있으면
            그것은 유예가 아니라 급여 폐지다.

     ★ **에이미는 그래도 떠나지 않는다**(→ wiki/officer.md). 유예는 그 사람의 이탈을 막는
       장치가 아니라 **선원의 이탈을 막으려고 그 사람에게 빌리는** 장치다. */
  defer: {
    /* ★★ **값은 실측이 두 번 뒤집어 잡았다** — 두 번 다 「선택지인가」를 물었다.
         1차 +6%p(유예 중에만)  ⇒ 이탈 44/60판 → 4/60판인데 대가가 213닢. **안 미룰 이유가 없다.**
         2차 +2%p/달 · 영구      ⇒ 대가 9,476닢 : 아낀 짐 2,676닢. **미룰 이유가 없다.**
         3차 +1%p/달 · 연속 두 달   ⇒ **횟수 제한이 제한이 아니었다.** 한 달 걸러 미루면
             여섯 달 중 3.88달을 미뤘고, 지분이 상한에 닿은 뒤로는 이자만 물면 그만이었다.
             ⇒ 문지기를 횟수에서 **지분 상한**으로 옮겼다(평생 두 번).
         4차 **+0.5%p/달 · 평생 두 번**(채택) ⇒ 대가 1,579닢 : 아낀 짐 1,609닢 —
             **짐만 세면 딱 눈금 위**이고, 여기에 **값이 안 매겨진 것이 둘** 얹힌다:
             갑판이 빠지면 배가 못 뜬다는 것(`shorthanded`)과, 초반의 이탈은 판을 끝낸다는 것.
             그 둘 때문에 「가려서 미루는 쪽」이 조금 이득이다 — 에이미 본인의 승률 6할과 같은 결이다.
         ⇒ 재조정할 때는 `node .playtest/round-27/g-probe-defer.mjs 60`의 마지막 두 줄
           (「아낀 짐 − 더 문 값」)을 본다. 한쪽이 두 배로 벌어지면 그 손잡이는 죽은 것이다. */
    rate: 1.15,       // 미룬 삯이 한 달마다 부는 배수
    share: 0.005,     // 한 달 미룰 때마다 **영구히** 오르는 성과급 (state.officer.share)
    shareMax: 0.01,   // 그 영구 인상의 상한 — 11% → 최대 12%. **이것이 곧 횟수 제한이다**(평생 두 번)
  },
...
export const TAVERN = {
  ...
  /* ── 평판 (2026-08-30 · 회차 27) ─────────────────────────────
     **체불이 오래가도 어느 항구에서든 똑같이 태울 수 있었다.** 급여를 못 주고 사람이 떠나도
     다음 술집에 앉은 무리는 그 사실을 몰랐다 — 벌이 *그 자리에서* 끝나 버려
     「싸게 태운 대가가 나중에 온다」는 이 화면의 설계가 **선원 쪽으로만 작동**했다.

     ── 무엇을 남기나 ────────────────────────────────────────────
     항구마다 자국(0~1)을 남긴다. 급여일에 못 준 비율만큼(`miss`), 사람이 떠나면 더(`desert`).
     ★ **소문은 바다를 탄다** — 같은 권역의 다른 항구도 그 절반을 안다(`spread`).
       한 항구만 더럽히고 옆 항구에서 태우면 그만이라면 규칙이 아니라 우회로다.
     ★ **읽을 때 계산한다**(`halfLife`) — 날마다 깎는 후크를 안 단다.
       이 저장소는 *세계가 도는 입구가 둘*(`advanceDays`·`waitDays`)이라 한쪽만 걸면
       그 층이 통째로 멈춘다(회차 26의 상단이 그랬다). 읽는 쪽에서 날짜로 환산하면 그 함정이 없다.

     ── 무엇이 달라지나 (세 겹) ──────────────────────────────────
       ⓐ `seats`  자리가 준다 — 소문난 배에는 사람이 덜 온다
       ⓑ `adv`/`wage` 값이 오른다. **계약금이 더 크게 오른다** —
          나중에 준다는 말을 못 믿으니 **선불을 더 달라**는 것이고, 이 시대 관행 그대로다
       ⓒ ★ **오는 사람이 갈린다.** 참을성 있는 무리(temper ≥ `sourAt`)가 걸리면 자국만큼의
          확률로 다시 굴려 **참을성 없는 쪽으로 기울인다.** 나포항의 재굴림(`prizeYard`)과
          **같은 수법**이다 — 도시별 확률표를 새로 만들지 않는다.
          ⇒ 체불하면 **참을성 없는 사람만 온다 → 다음 체불이 더 아프다**. 나선이 생긴다.

     ⚠️ **자리를 0으로 만들지 않는다.** 술집이 비면 배가 묶여 막다른 길이 된다 —
       하한 1자리를 두고, 그래도 못 태우면 부두 인부(`HIRE_UNIT`)가 마지막 문이다(§7). */
  rep: {
    miss:     0.45,   // 급여를 통째로 못 준 달에 그 항구에 남는 자국 (못 준 비율에 비례)
    desert:   0.30,   // 무리 하나가 짐을 들고 떠날 때마다 더해지는 자국
    cap:      1.00,   // 자국의 상한
    halfLife: 90,     // 며칠에 절반으로 옅어지나 (한 판이 900~1,100일이므로 잊히되 늦게 잊힌다)
    spread:   0.50,   // 같은 권역의 다른 항구가 아는 몫
    seats:    2,      // 자국 1.0에서 줄어드는 자리 수
    adv:      0.60,   // 자국 1.0에서 계약금 +60%
    wage:     0.20,   // 자국 1.0에서 요구 일당 +20%
    sourAt:   0.70,   // 이 temper 이상이면 「참을성 있는 무리」 — 자국만큼의 확률로 다시 굴린다
  },
};
...
/* ── 자리(座) — 명(明)의 지방 관리에게 치르는 관례(常例) ────────────────
   §A-11 명. 사용자 지시는 *"중국의 경우 뇌물로 각 지방 항구 관리자 이용해서 수익을 얻는
   형태로"*였다. 조사가 내린 결론은 **「뇌물」보다 「관례(常例)·누규(陋規)」**다 —
   명·청의 관인 봉급은 1392년에 고정된 뒤 물가와 무관하게 유지되어 극히 낮았고, 법정 세
   바깥의 관행 수수료로 관아가 돌아갔다. **그것 없이는 관아가 돌아가지 않는다는 것을
   다들 알았다.** 그래서 이것은 부정을 저지르는 재미가 아니라 **이 바다의 셈법**이고,
   「매번 값이 든다」가 벌이 아니라 비용으로 읽힌다.

   ★ 사료적 원형 — 1553~54년 왕백(汪柏, 海道副使). 포르투갈이 지방 관리 **한 사람**에게
     값을 치르고 광주에서 통관하게 되었고 그것이 마카오가 되었다.
     ⇒ **단위가 「관리 하나 · 항구 하나」**인 근거이고, `days`(임기)와 `step`(누진)이 여기서 나온다.

   ★★ **자리가 여는 것은 「수입」이 아니라 「지출할 자격」과 「일감의 크기」다.**
     거점을 정가에 사고(자리가 없으면 할증), 그 항구의 대형 주문이 커진다 —
     **보수를 받으려면 실어 날라야 한다.** `FACTIONS` 머리주석의 방어선
     (*"관계가 좋아져도 원래보다 더 벌지 않는다"*)을 그대로 지킨다.
   ⛔ **세율은 한 자리도 안 건드린다.** 실측으로 죽어 있다 — 광저우(7.5%)는 부관 −35% +
     감합 −35% + 밀수 −30%면 이미 바닥(`BOON.tariffFloor` × 원세 = 1.875%)이라 **네 번째
     감면은 아무것도 안 움직인다.** 그리고 `check-factions` ⑤가 이중과세를 금지한다.
   ⛔ **막지 않고 값을 물린다.** 자리를 「거점의 필수조건」으로 만들면 동아시아 패권 ①
     (41항구 전부 거점 · 그중 13이 명)이 통째로 이 기능에 잠긴다 — 게임이 어려워지는 것이
     아니라 **막히는 것**이고 그 차이는 러너 로그로 안 읽힌다. 삭은 배·계절창·후반
     브레이크에서 세 번 고른 답과 같다. */
export const SEAT = {
  /* 값 — `FACTION.enrollBySize 900`을 그대로 쓴다(자를 새로 만들지 않는다).
     광저우(size3) 2,700 → 3,915 → 5,677 … · 톈진(size2) 1,800 → 2,610 … · 펑후(size1) 900.
     ⚠️ 사료가 액수를 안 준다(누규의 금액 사례를 못 찾았다) — **게임 내부의 자에서 역산한 값**이다. */
  bySize: 900,
  step: 0.45,            // 두 번째부터 ×1.45씩 — 「돈을 넣으면 돈이 나오는 기계」를 막는 첫 장치
  days: 180,             // 임기. 감합·인표가 해마다 갱신되던 것에 맞춘다(permit 45 · enroll 90보다 길되 영구가 아니다)
  /* 자리가 없을 때 거점값에 붙는 할증. **자격이 아니라 값이다** — 못 사는 것이 아니라 비싸다.
     ⚠️ 일본(무력 개항)이 「자격」을 쥐므로 명은 반드시 「값」이어야 서로를 안 밟는다. */
  noSeatUp: 0.35,
  contractUp: 0.25,      // 자리가 있는 항구의 대형 주문 보수
  /* 같은 성(省)의 이웃으로 번지는 할인 — 「한 관리를 산 소문이 옆 고을로 간다」.
     `TAVERN.rep.spread 0.5`(같은 권역으로 절반 번진다)가 이미 쓰는 패턴이다. */
  spread: 0.35,
  provinces: {
    guangdong: ['guangzhou', 'chaozhou', 'qiongzhou'],
    fujian:    ['quanzhou', 'yuegang', 'fuzhou', 'penghu'],
    zhejiang:  ['ningbo', 'wenzhou'],
    nanzhili:  ['nanjing', 'shanghai'],
    beizhili:  ['dengzhou', 'tianjin'],
  },
  /* 대가 — 감찰(監察). 살아 있는 자리가 많을수록 어사의 눈에 든다.
     걸리면 그 항구의 자리가 **환불 없이** 사라지고 시박사와의 사이가 상한다. */
  auditPer: 0.02,        // 살아 있는 자리 하나당 입항 판정 확률
  auditCap: 0.14,
  auditRegard: -2,
};
...
export const FACTION = {
  ...
  /* ── 상단 ↔ 세력 (2026-08-30 · 회차 27 · 다-4) ─────────────────
     **두 층이 서로를 모르고 있었다.** 세력은 정치·독점이고 상단은 자본·물류인데,
     한 항구에서 그 둘이 같은 물건을 놓고 만나면 **서로를 먹어야** 한다 —
     안 그러면 「독점」과 「물가 안정화」가 같은 도시에서 각자 온전히 성립한다.

     ★★ **경계 규칙은 그대로다.** `test-rules`가 감시하는 것은
       *"`js/npc/guild.js` 소스에 `gripMarkup`·`rollPoach`가 없을 것"* —
       **두 층이 서로의 손잡이를 베끼지 말라**는 뜻이지 만나지 말라는 뜻이 아니다.
       그래서 만나는 자리를 **`state.js` 한 곳**으로 정했다. 상단 엔진도 세력 규칙도
       한 줄 안 바뀐다 — 값이 곱해지는 자리만 둘 늘었다.

     ── 두 방향 ────────────────────────────────────────────────
       ⓐ **독점은 자본을 밀어낸다**(`gripBlocksGuild`) — 세력이 쥔 항구·품목에서는
          상단이 미는 힘이 그만큼 덜 먹는다. 특허 독점은 남의 자본을 못 들어오게 하는 장치였다.
          ⇒ **세력이 만든 값차이가 상단 때문에 무뎌지지 않는다.** 이 저장소가 세력 설계에
            못박아 둔 *"답이 언제나 딴 데서 사는 것(=항로가 길어진다)"*이 그대로 산다.
       ⓑ **자본은 정치를 얕게 만든다**(`guildCounter`) — 큰 상관이 여럿 앉은 항구에서는
          웃돈이 깎인다. 창고에 남의 물건이 쌓여 있으면 임자가 값을 온전히 못 부른다.
          ⇒ **어느 상단이 흥하는지가 플레이어에게 처음으로 값이 된다**(그전에는 구경거리였다).

     ⚠️ ⓑ는 `buy()`의 이분 탐색 때문에 **수량과 무관해야** 한다 — 항구와 그날의 자본만 본다. */
  gripBlocksGuild: 0.55,     // 쥔 자리에서 상단의 물가 좁힘이 이만큼 덜 먹는다
  guildCounter: 0.40,        // 상관이 두꺼운 항구에서 웃돈이 최대 이만큼 깎인다
  guildCounterRef: 200_000,  // 그 감쇠의 반환점 자본(닢) — 중앙 상단(63,000닢) 둘쯤
  ...
  /* ── 회차 28 · 상단과 세력이 만나는 자리 **셋째·넷째** — 이번에는 값이 아니라 **관계**다 ──
     회차 27은 둘을 값에서 만나게 했다(`gripBlocksGuild`·`guildCounter`). 그것만으로는
     **상단을 건드린 대가가 상단 안에서만 끝난다** — 프리울리 상관의 호위선단을 꺾어도
     같은 깃발을 제 것이라 부르는 베네치아 원로원은 아무것도 몰랐다.
     ⚠️ **둘 다 뺄셈이라 새 수입원이 아니다**(A-10이 3단계까지 지킨 원칙).
     ★ 경계는 그대로다 — `js/npc/guild.js`는 한 줄도 안 바뀌고, 만나는 자리는 `state.js` 하나다. */
  backerRaw: -1,         // 상단의 호위선단을 꺾었다 → 그 **깃발** 뒤에 선 세력이 등을 돌린다
  /* ★ 세력의 함대(−4)보다 가볍다 — 꺾은 것이 나라의 배가 아니라 **그 나라 깃발을 단 회사의 배**다.
     깃발이 없는 세력(푸거)은 이 자리에서도 조용하다: 덮칠 배가 없으면 등을 돌릴 일도 없다. */
  stirRaw: -1,           // 남의 사주로 남의 자리를 흔들었다 → 그 **마당의 임자**도 소란을 본다
  /* ★ 사주의 삯(`GUILD` 쪽 `fee`)은 한 닢도 안 건드린다 — 값이 하나 붙는 것이지 벌이가 주는 것이 아니다.
     설계가 사주를 *"주인공이 남의 싸움의 도구가 된다"*로 적어 두었는데, 그 도구질이
     임자의 부두에서 벌어지는데도 임자만 모르고 있었다. */
  ...
  /* ── 3단계 · 회사의 벌목 (`sells: 'nothing'`) ────────────────────────
     ★ **파는 대신 벤다.** 값을 지키는 방법이 파는 것을 줄이는 것이 아니라 자라는 것을 없애는 것이다.
     ⚠️ **새 수입원이 아니다** — 산지 값이 **오르므로 사는 쪽이 손해**이고, 수요지 값은
       한 톨도 안 건드리므로 **팔 때 이득이 되는 자리가 없다.** */
  fellAt: -3,            // 이 아래로 내려가면 벤다
  fellEvery: 60,
  fellMult: 1.35,        // 산지 값이 오른다(공급 충격)
  fellDays: 60,

  /* ── 3단계 · 세력끼리의 나포 — **목격할 뿐이다** ──────────────────────
     ⚠️ 플레이어에게 아무것도 안 주고, **상선 정원을 안 줄인다**(털린 배는 다시 채워진다).
     기존 `raid` 충격을 그대로 재사용해 **시세 사건으로만** 남는다. */
  raidPerDay: 0.012,     // 하루 확률 — `NPC.raidBase`와 따로 둔다
  raidMult: 1.28,
  raidDays: 14,
};
...
/* ── 항구 거점 (A-1) ───────────────────────────────────────────
   ...
   ★ **「부두」는 여기 없다**(C-18 · 2026-08-28 설계 변경). 사용자 지시:
     *"부두는 거점이 될수 없고 (…) 그 국가에 이익이 쌓이면 차근차근 국가에서 조선소 부두를
     만들고 (…) 거점은 국영말고 다른것들이 되어야지 번화가 상점 여관같은거 말야."*
     ⇒ 공업력은 **내가 사는 것이 아니라 그 항구에 낸 세로 나라가 짓는 것**이 됐다(`CIVIC`).
       조선대(`slipway`)는 남는다 — **내 배를 손보는 민간 시설**이지 나라의 조선소가 아니다.
...
export const HOLDINGS = {
  ...
  /* ── 수익형 부동산 (#5) ──────────────────────────────────────
     ★ 위 넷은 전부 **내 무역을 돕는** 시설이다 — 짐을 두고, 세를 깎고, 배를 손본다.
       ...
       ② **승급에 공업력이 든다**(`industry`). 나라가 지은 조선소(`CIVIC`)와 A-2 승급으로
          올린 공업력이 여기서 두 번째 쓸모를 얻는다 — 사용자 원문 *"승급은 공업력으로 올려도 되고"*.
  ...
  /* ── 번화가 (C-18 · 2026-08-28) ────────────────────────────────
     사용자가 거점의 자리를 다시 그으며 **셋을 콕 집었다** — *"번화가 상점 여관같은거 말야."*
     `shop`·`inn`은 이미 있었으므로 없던 하나를 세운다.

     ★ **가게·여관과 무엇이 다른가** — 가게는 한 칸이고 여관은 한 채인데, 번화가는 **줄**이다.
       세를 내는 사람이 여럿이라 **한둘이 비어도 그 달이 0이 되지 않는다** — 그래서
       공실이 가장 낮고(가게보다도), 대신 **값이 자릿수로 크고 만실 수익률은 가장 얕다.**
       ⇒ 셋의 성격이 갈린다: 가게는 완만 · 여관은 도박 · **번화가는 무겁고 느리다.**
       이것이 후반 금화가 갈 자리다(옛 부두 100,000~140,000닢이 사라진 그 자리).

     ★ **공업력이 문이다**(1·2·3). 나라가 조선소를 올린 항구라야 줄이 선다 —
       ②(`CIVIC`)와 ③(부동산)이 여기서 맞물린다. 사용자 원문 *"해당국가 공업력을 올리면"*이
       조선소만이 아니라 **그 항구가 커지는 것** 전부를 뜻하게 된다.

     ── 근거 ──────────────────────────────────────────────────
     나라가 큰길에 행랑을 지어 상인에게 세를 놓는 제도는 실재했다 — 조선 태종대 종로
     **시전 행랑**(관이 지어 시전에 임대), 베네치아 리알토의 **botteghe**(공화국 소유 점포 줄),
     오스만의 **베데스텐/한**(와크프 임대수입). 순 연수익률은 가게·여관과 같은 밴드에 둔다.
     → content/asset-evidence.json: shopRow */
  bazaar:   { name: '번화가',    store: 0, estate: true,
              desc: '큰길가 행랑을 통째로 쥔다. 세 내는 사람이 여럿이라 한둘이 비어도 굴러간다.',
              grades: [
                { name: '골목 행랑', priceBase: 9000,  priceBySize: 7000,  yield: 0.098, vacancy: 0.05, industry: 1 },
                { name: '시전 행랑', priceBase: 30000, priceBySize: 22000, yield: 0.120, vacancy: 0.09, industry: 2 },
                { name: '번화가',    priceBase: 80000, priceBySize: 60000, yield: 0.143, vacancy: 0.15, industry: 3 },
              ] },
};
...
export const HOLDING = {
  ...
  /* ⚠️ `industryCap`은 여기 없다 — **`CIVIC.cap`으로 옮겼다**(C-18 · 2026-08-28).
     거점이 공업력을 올리던 시절의 상한이라, 부두를 거점 목록에서 뺀 지금은 나라 쪽 값이다. */
...
/* ── 나라가 짓는 조선소 (C-18 · 2026-08-28) ─────────────────────
   ★ 사용자 지시로 **공업력을 올리는 길이 바뀌었다.** 전에는 거점 「부두」를
   60,000 + 40,000×공업력 닢에 **사면** 그 자리에서 +1이었다. 지금은 살 수 없다:

     *"특정국가에 항구에 물건을 사고팔고 그 국가에 이익이 쌓이면 차근차근 국가에서
       조선소 부두를 만들고 해당국가 공업력을 올리면 되는거지"*

   ── 무엇을 지표로 삼나 ──────────────────────────────────────
   **그 항구에 낸 세다**(`state.dues[cityId]`). 후보가 셋이었다:
     ⓐ 그 항구에서 낸 매출 — 내 이익이지 *그 나라의* 이익이 아니다
     ⓑ 거점 투자액 — 사용자가 방금 부정한 「돈으로 사는 것」이 이름만 바꿔 돌아온다
     ⓒ **입항세·관세 납부 누적** ← 채택. 사용자 원문 *"그 국가에 이익이 쌓이면"*에 가장 가깝고,
        이미 있는 배선(`tariffRate`·`baseTariff`)에 얹으므로 **새 수입원을 만들지 않는다.**
        ⚠️ 절대 원칙: 돈은 어렵게 벌려야 한다 — 이 규칙은 한 닢도 주지 않는다.

   ── 왜 항구별인가 (국가별이 아니라) ─────────────────────────────
   `industryOf(cityId)`·`ENDING.yards`(염포 3 · 부산포 3)·`tierNeeded`가 **전부 도시 단위**라
   깃발 단위로 쌓으면 부산포에서 판 것이 마포·의주까지 동시에 올린다. 「차근차근」이 사라진다.
   화면에는 *"이 항구에 낸 세"*라고 그대로 적는다.

   ── 무게 (밸런스 앵커) ──────────────────────────────────────
   옛 부두는 `60,000 + 40,000×공업력`이었다(염포 실측 100,000~140,000닢).
   후반 실효 관세율이 약 9.36%이므로 세 100,000닢은 거래 약 1,070,000닢어치다.
   ⚠️ **그 숫자를 그대로 쓰지 않았다** — 실측(`.playtest/round-22/probe-dues.mjs`)으로
   「한 항구에 실제로 쌓이는 속도」를 재서 아래 값을 정했다. 세는 매출의 일부라
   거래액 전부가 한 항구에 몰리지 않는다(사는 항구와 파는 항구가 다르다).

   `dues[n]` = 지금 공업력이 n일 때 **다음 칸**을 열기까지 그 항구에 **쌓여야 하는** 세(누적).
   `days[n]` = 그 공사에 걸리는 날. *"차근차근"* — 문턱을 넘어도 즉시가 아니다.

   ⚠️ **「쌓인 것」과 「낸 것」은 이제 다르다**(아래 `spill`). 한 항구에 100,000닢을 내면
     그 항구에 돌아오는 것은 **53~61%**뿐이고 나머지는 그 나라의 다른 항구로 흘러간다(실측).
     ⇒ 「국가를 거쳐 항구로」를 넣으면서 문턱을 **0.55배로 되돌려 곱했다** —
       그래야 *납부액* 기준 앵커(약 30,000닢 = 옛 부두 한 칸)가 그대로 유지된다.
       염포 30,800닢 납부 · 부산포 27,200닢 납부로 각각 3에 닿는다(실측 회수율 53.6%·60.7%).
     **이 값을 만질 때는 `spill.keep`도 함께 봐야 한다** — 둘이 곱해져 한 수가 된다. */
export const CIVIC = {
  cap: 3,               // 도시 `industry` + 나라 조선소가 넘을 수 없는 선 (옛 `HOLDING.industryCap`)
  dues: [2000, 6500, 16500, 33000],    // index = 지금 공업력 (3 이상은 상한에 걸려 안 쓰인다)
  days: [90, 150, 240, 360],           // 그 공사에 걸리는 날

  /* ── 국가를 거쳐 항구로 (2026-08-28 · 사용자 결정) ────────────────
     ISSUES #1(*"항구별이냐 국가별이냐"*)에 사용자가 **셋째**를 골랐다: *"둘 다 — 국가를 거쳐 항구로."*
     ⇒ 낸 세는 **두 갈래로 갈린다**: 절반은 낸 그 항구에 그대로 남고(`keep`),
       나머지는 **그 나라의 몫**이 되어 그 깃발 항구들에 나뉘어 흘러든다.

     ── 왜 「순위 감쇠」가 아니라 「규모 가중」인가 (설계상 함정 하나) ──────────
     ⚠️ 처음에 *"주항구가 먼저"*를 **등수마다 감쇠**로 옮기려 했다가 **조선의 끝을 막을 뻔했다.**
       조선 아홉 항구를 (공업력, 규모)로 줄 세우면 **염포가 7등**인데(`industry 1 · size 1`),
       `ENDING`은 그 염포를 공업력 3까지 올리라고 요구한다. 감쇠를 쓰면 염포 몫이 2.8%가 되어
       나라 쪽 길이 사실상 닫힌다. **줄 세우기는 목표와 어긋날 수 있다.**
     ⇒ 가중치는 **도시 `size`**로 준다. 이 게임에서 size는 이미 *"사람과 돈이 얼마나 도는 곳인가"*의
       축이고(입항세·시장 깊이·공실이 전부 그것을 본다) **정적 데이터**라 배분이 흔들리지 않는다.
       *"주항구가 먼저"*는 `mainBonus`가 맡는다 — 그 나라에서 (공업력, 규모)가 가장 높은 한 곳에만 얹는다.
     ★ **몫이 줄어드는 일은 없다.** 가중치는 정적이고, 나뉘는 대상에서 빠지는 것은
       **상한에 닿은 항구뿐**이다(한 번 닿으면 되돌아오지 않는다) — 남은 항구의 몫은 늘기만 한다.
       줄어들 수 있으면 이미 선 공사가 취소되는 자리가 생긴다.

     ── 「차근차근」이 사는 이유 ────────────────────────────────
     내가 직접 낸 몫(`keep`)이 여전히 절반이라 **내가 무역하는 항구가 가장 빨리 오른다.**
     나라 몫은 트리클이다 — 부산포에서 번 것이 마포·의주를 *조금씩* 밀어 올릴 뿐,
     아홉을 동시에 올리지 않는다. 그리고 **총량은 보존된다**(세를 새로 만들지 않는다).
     ⓘ 큰 제국일수록 나라 몫이 얇게 퍼진다(스페인 66항구 ↔ 류큐 1항구) — 의도한 성질이다.
       작은 나라는 몰아서 오르고 큰 나라는 천천히 오른다. */
  spill: {
    keep: 0.50,        // 세를 낸 그 항구에 그대로 남는 몫. 나머지가 나라 몫이다
    mainBonus: 1.0,    // 주항구에 얹는 가중치 (가중치 자체는 도시 `size`)
  },
};
...
/* ── 작위(爵位)와 개항 — 조선의 문 ────────────────────────────────
   §A-11 조선. 사용자 지시: *"한반도 주인공이 한반도 왕에게 미션을 받고 수행하여 한반도 개항하고
   작위를 받아서 한반도 전체 공업력을 올리고 개항이벤트를 열수 있게 하자 최소 몇개지역 패자되면 어때"*

   ★★ **「개항」은 무언가를 여는 일이 아니라 먼저 「닫혀 있음」을 만드는 일이었다** — 실측으로
     조선 9항구를 막는 것이 코드에 **하나도 없다**(항로·관세·세력 어느 층에도 잠금이 없고,
     "열린 부두가 하나뿐"은 주석과 blurb의 *서술*이다). 다만 **데이터에는 흔적이 있었다**:
     NPC 상인 68명 중 조선 항구를 순회로에 넣은 것은 **둘뿐**이고 닿는 곳도 마포·부산포 둘뿐이다.
     ⇒ **「바깥 배가 안 온다」가 이미 데이터로 표현돼 있다.** 개항은 그 자리를 연다.

   ★ 고증 — **1876년식 근대 개항을 가져오면 시대가 어긋난다.** 16~17세기 조선이 연 것은
     항구가 아니라 **「장(場)」**이었다: 삼포개항(내이포·부산포·염포 — **셋 다 게임에 이미 있다**) ·
     **개시(開市)**는 상시 개방이 아니라 **1년 몇 회로 정해진 정기 시장**이었다(중강 1593 ·
     회령·경원 1645). 그래서 개항 이벤트는 「한 번 열고 끝」이 아니라 **되풀이되는 장**이다.

   ★★ 작위는 **공명첩(空名帖)·납속(納粟)**이다 — 곡식을 바치면 품계를 적어 주되 **이름 칸이 비어 있고**
     실권이 없는 명예직(`납속`·`가설`이 붙어 정식 관작과 구별됐다). 이 게임에 이보다 잘 맞는
     제도가 없다: ⓐ 공업력이 이미 **「그 항구에 낸 세」**로 오른다(`CIVIC`) — 같은 원리다
     ⓑ **실권이 없다**는 것이 `HEGEMONY`·`FACTIONS`의 *"보상으로 돈이나 수입을 주지 않는다"*와 겹친다
     ⓒ ★ **서사가 닫힌다** — 납속의 **서얼허통**(서얼의 관직 진출 허용)이 `interpreter`(역관의 서자 ·
       *"문서에 제 이름을 못 올린다"*)와 `bastard`(재상가 서자 · 서얼금고법)의 **대가를 정면으로 푼다.**
       ⇒ **작위는 「이름을 얻는 것」이다.**

   ⛔ **금화·수입·보수를 한 닢도 주지 않는다.** 주는 것은 **천장과 속도와 이름**뿐이다.
   ⛔ **조정을 `FACTIONS`에 넣지 않는다**(§A-11 ㄴ) — `check-factions`가 `busanpo: null`을
     명시적으로 잠갔고 소설 28·64·68장이 거기 걸려 있다. 왕의 뜻은 **인물**이 전한다.
   ⛔ **「동아시아를 반드시 포함」으로 적지 않는다**(§A-11 ㅁ) — 동아시아 ④는 철갑 거북선(염포
     공업력 3)이라, 작위가 그 공업력을 준다면 **작위를 받으려면 작위가 필요한 고리**가 된다. */
export const ROYAL = {
  /* 계단 셋. **어느 바다든 상관없다**(위 ⛔). N=3이 첫 칸인 근거는 §A-11 ㅁ —
     1은 공짜(조선 갈래의 자연 목표가 이미 동아시아다) · 9는 `markNineEnded`와 중복 ·
     3이면 **원양 관문(광저우→믈라카)을 반드시 넘는다.** 5·7은 아직 아무도 못 본 자리이고,
     그래서 **첫 칸에 이 기능의 핵심(이름과 개항)을 다 둔다** — 못 볼 보상은 보상이 아니다. */
  steps: [
    { need: 3, title: '통정대부', rank: '정3품 당상',
      gain: '이름을 얻는다 — 조선 아홉 항구가 바깥 배에 열리고 관세가 가벼워진다' },
    { need: 5, title: '가선대부', rank: '종2품',
      gain: '나라가 올릴 수 있는 공업력의 천장이 한 칸 열린다 (3 → 4)' },
    { need: 7, title: '첨지중추부사', rank: '중추부 — 사무 없는 관청',
      gain: '나라 조선소의 문턱이 가벼워진다 (낸 세의 기준 −25%)' },
  ],
  /* 2계단이 조선 깃발에만 여는 천장. ★ **새 값을 하나도 안 만든다** —
     `CIVIC.dues[3]=33000`·`days[3]=360`이 이미 있고 주석이 *"3 이상은 상한에 걸려 안 쓰인다"*라
     적어 둔 **죽은 값**이었다. 이 한 칸이 그것을 살린다. */
  civicCap: 4,
  duesOff: 0.25,        // 3계단 — `CIVIC.dues` 문턱을 이만큼 깎는다
  /* 1계단(개항)이 조선 항구에 주는 관세 감면. ⚠️ `BOON.tariffFloor` 바닥은 그대로 적용된다 —
     제도는 피해 갈 수 있어야 하지만 없어지면 안 된다. */
  openTariffOff: 0.15,
  /* 개시(開市) — 나라가 여는 장. **상시가 아니라 주기**다(사료가 「1년 2회」 꼴이다).
     `SHOCK.events`의 되풀이 사건으로 걸고, 열리면 조선 항구의 수요가 오른다. */
  fairEveryDays: 180,
  fairDays: 20,
  fairDemand: 0.18,
};

/* ── 다이묘의 수군 — 힘으로 여는 문 (§A-11 일본) ────────────────────
   사용자 지시: *"일본의 경우 강제 개항 이벤트 해군력이 일본 전역보다 세지면"*.

   ★★ **고증이 설계를 바꿨다 — 이 시대 일본에는 「전역 해군」이 없었다.** 전역이란 다이묘 수군의
     합이고, 그것이 하나가 되는 사건이 1588년 해적정지령이다. 그래서 판정도 **다이묘별**이다:
     쓰시마가 먼저 열리고, 히라도·나가사키·세토내해가 그다음이고, 사쓰마·기나이가 마지막이다.
     **한 번에 열리면 「아 그렇구나」로 끝난다** — 항구가 하나씩 열려야 힘이 자란 것이 손에 남는다.
   ⚠️ **무력이 실제로 이 시대 일본의 항구를 연 사례는 없다.** 1565 후쿠다만은 다이묘가 무력으로
     *닫으려 한* 사건이고 포르투갈은 이겼지만 항구를 못 얻었다 — 나가사키는 1571년 오무라가
     **내주었다**. 네덜란드(1609)는 초대장으로 들어왔다. 무력이 무역항을 연 것은 1609년 시마즈의
     류큐 침공 하나이고 **일본이 남을 친 쪽**이다. ⇒ 그래서 여는 것도 「입항」이 아니라
     **「그 다이묘가 나를 상대로 인정하는 것」**이다 — 배를 짓게 하고 상관을 두게 한다.

   ⛔⛔ **입항을 막지 않는다. 거점도 막지 않는다.** `hegemonyOf('eastasia').ports.need = 41`이고
     그 41에 일본·류큐 13항구가 들어 있다 — 입항이나 거점을 닫으면 **동아시아 패권 → 아홉 바다
     엔딩이 통째로 이 기능에 묶인다.** 닫는 것은 **조선소(`slipway`)와 상관(`factory`)뿐**이고,
     상관 3은 일본 밖에서도 채워지므로 패권 ②도 안 막힌다.
   ⇒ **명은 「값」(거점 할증) · 일본은 「자격」(조선소·상관)** — 같은 손잡이에 하나는 계수로,
     하나는 게이트로 붙어야 서로를 안 밟는다(§A-11 · 두 기능이 같은 회차에 들어갔다).
   ⛔ **항로를 지우지 않는다** — `check-world` ①이 정적으로 265항구 도달성을 본다. `state` 게이트로만.

   ★ `strength`는 **표에 손으로 적는다**(`PIRATES[].strength`가 그렇듯). 아래 「근거」는 그 값을
     정한 계산이지 규칙이 읽는 값이 아니다 — `size+industry`를 그대로 읽으면 도시를 늘릴 때마다
     조용히 바뀐다. */
export const DAIMYO = {
  /* 문턱은 **패권 ③(등급5)보다 낮게** 둔다 — 최고가 3이라 «일본을 열고 → 그 힘으로 패권을 노린다»는
     순서가 생긴다. 같은 자리에 두면 둘 중 하나가 사족이 된다. */
  clans: [
    { id: 'so', name: '쓰시마의 소씨', strength: 1, ports: ['tsushima'],
      why: '섬 하나의 영주다. 계해약조로 세견선 쉰 척을 얻은 것이 이 집의 전부다 (size+ind 2)',
      line: '“섬 하나 가진 집이오. 당신 배를 막을 힘은 없소 — 대신 문서는 우리를 거치시오.”' },
    { id: 'matsuura', name: '히라도의 마쓰우라씨', strength: 2, ports: ['hirado'],
      why: '왕직을 받아 준 영주. 1565년 후쿠다만에서 포르투갈과 싸웠다 (3)',
      line: '“여기 배를 대던 사람이 누구였는지 아시오? 왕직이오. 우리는 사람을 가리지 않소.”' },
    { id: 'omura', name: '나가사키의 오무라씨', strength: 2, ports: ['nagasaki'],
      why: '1571년 나가사키를 내주어 이 바다에서 가장 큰 항구를 만들었다 (5)',
      line: '“이 항구는 우리가 내준 것이오. 내줄 만한 상대인지는 우리가 보오.”' },
    { id: 'murakami', name: '세토내해의 무라카미 수군', strength: 2,
      ports: ['onomichi', 'akamagaseki', 'tsuruga', 'sado'],
      why: '좁은 목에서 통행세를 걷던 바다의 영주. 명부에 이미 있다(`murakami`) (12)',
      line: '“이 목은 우리 것이오. 지나려면 값을 내거나, 값을 안 낼 만큼 세거나.”' },
    /* ⚠️ **하카타는 여기서 뺐다** — 규슈의 상인 도시이고 기나이(사카이·오사카)와 멀다.
       이 게임에도 가미야 소탄·시마이 소시쓰가 앉아 있는 «상인이 다스리는 항구»이지
       수군의 항구가 아니다. 처음에 구키에 넣었다가 고증으로 뺐고, 그 덕에 동아시아
       초반 조선소 하나가 살아 있다(닫는 것은 적을수록 좋다). */
    { id: 'kuki', name: '기나이의 구키 수군', strength: 3, ports: ['sakai', 'osaka'],
      why: '1578년 제2차 기즈가와구치의 철갑선. 그 배가 이미 게임에 있다(`tekkosen`) (11)',
      line: '“쇠를 붙인 배를 보신 적 있소? 우리가 만들었소.”' },
    { id: 'shimazu', name: '사쓰마의 시마즈씨', strength: 3, ports: ['kagoshima', 'bonotsu'],
      why: '1609년 배 백 척과 삼천으로 류큐를 쳤다 — 이 시대 무력이 실제로 항구를 연 유일한 사례 (6)',
      line: '“우리는 남의 나라를 친 적이 있소. 그쪽은 어떻소?”' },
  ],
  /* 나하(류큐)는 다이묘가 없다 — **1609년 그대로** 시마즈를 넘으면 함께 열린다. */
  ryukyuUnder: 'shimazu',
  /* ★ **대가는 따로 물리지 않는다 — 전력을 갖추는 것 자체가 값이다.**
     처음에 악명(`state.infamy.japan`)을 얹으려 했으나, 문턱을 넘어 열리는 것은 *싸움이 아니라
     인정*이라 악명이 붙을 자리가 없다. 실제로 수군을 꺾어서 열면 그 전투가 이미 악명을 물리므로
     여기서 한 번 더 물리면 이중과세다.
     ⚠️ 그리고 악명을 얹으면 **「연 뒤에 못 쓰는 문」**이 된다 — 세율 ×2.6 · 조우 +12%p가
       열자마자 걸려 *연 것이 손해*가 된다. 40일 감쇠가 눅이지만 애초에 안 붙이는 편이 맞다. */
};
...
/* ── 조우가 앗아가는 것 (2026-08-28 · 사용자 결정 「상한 + 선원 바닥」) ──────────
   ★ **바로 위 `SPOILS_*`의 거울이다.** 이 게임은 *내가* 얻는 전리품을 이미
     *"옮겨 실을 수 있는 만큼"*으로 누르는데(`state.js: capLoot`·`spoilsCap`),
     **내가 잃는 쪽에는 그 대칭이 없었다.** 그래서 손실이 금고의 비율이고 상한이 없었다 —
     패배 ×0.5(`scenes/battle.js`) · 싸우기 전 도주 ×0.12(`scenes/map.js`).

   ── 왜 고치나 (개발자 B 실측 · `.playtest/round-22/probe-purse.mjs`) ──────────
   비율 손실은 금고에 **자연 상한**을 만든다: `G* = 항차이익 / (조우율 × 손실률)`.
     항차 순이익 2,000닢 · 도주만 고르는 「올바른 플레이」  → 금고가 **46,425닢**에서 멈춘다
     항차 순이익 8,000닢                                  → 185,701닢
   그런데 **패권 거점 총투자가 912,630닢**이다. ⇒ 후반 목표의 문이 **구조적으로** 닫혀 있었다.
   ★ 급한 것은 패배 50%가 아니라 **도주 12%**다 — 그것이 올바른 플레이가 *언제나* 무는 값이라
     천장을 만드는 쪽이다(패배는 피할 수 있고, 피하는 것이 이 게임의 설계다).

   ── 상한의 모양 — 「적도 옮겨 실을 수 있는 만큼만 가져간다」 ────────────────
   ⚠️ **상한을 내 자산의 비율로 두면 아무것도 안 바뀐다**(비율에 비율을 곱한 것뿐이다).
     `spoilsCap()`이 *이긴 자의 크기*로 재듯이, 이쪽은 **적의 크기**로 잰다 —
     갑판에 열둘이 선 좀도둑이 내 금고 90만 닢을 통째로 나를 수는 없다.
       `cap = max(floor, 적 선원 수 × perCrew)`
       `잃는 값 = v <= cap ? v : cap + (v - cap) × tail`      ← `capSpoils()`와 같은 꼬리
   ★ **초반은 한 자리도 안 바뀐다** — 금고 200닢의 50%는 100닢이라 상한 아래다.
     어려움은 있던 자리에 그대로 두고 **후반의 천장만** 걷어낸다.
     *"돈은 어렵게 벌려야 재미가 있다"*를 안 해친다.

   ── 어느 손잡이가 무엇을 하나 (B에게 · 값을 고르기 전에 읽을 것) ────────────
   ★ **천장을 정하는 것은 `tail`이다.** 상한 위에서 실효 손실률이 `share × tail`이 되므로
     `G* = 항차이익 / (조우율 × share × tail)` — 곧 **`G*`가 `1/tail`배로 늘어난다**(0.10이면 10배).
   ★ `perCrew`는 천장이 **어디서부터** 걷히나를 정한다(상한에 닿는 금고 = `cap / share`).
   ★ `floor`는 **초반을 지키는 값**이다 — 이것이 크면 가난할 때가 더 아파진다. 낮게 둔다.
   임시값으로 재 본 것(등급별 상한 4,840~28,600닢):
     금고 200닢 패배 손실 **100닢 — 지금과 한 자리도 안 다르다** ✔
     금고 900,000닢 도주 손실 108,000 → **15,156~36,540닢**(등급에 따라)
   ⇒ 항차이익 2,000닢 기준 `G*`가 46,425 → 약 46만으로 열 배 올라간다. 패권 총투자 912,630닢에
     닿게 하려면 `tail`을 더 낮추거나(0.05면 스무 배) 항차이익이 커지는 후반 구간과 함께 봐야 한다.

   ── 선원 바닥 (잠기는 축은 금고가 아니라 사람이다) ──────────────────────
   선체는 패배해도 `maxHp`의 25%로 **되돌아갈 뿐 누적이 아닌데**, 선원은 ×0.5로 **누적된다**
   (42 → 21 → 11 → 6 → 4). `oceanReady()`가 사람이 모자라면 원양을 막고, 금고가 비면
   사람을 못 태운다 — 금고는 그 문의 열쇠일 뿐이었다. ⇒ 반감에 바닥을 준다.

   ⚠️ **값은 임시다 — 개발자 B가 실측으로 확정한다**(`sim-stat 20`의 10항차 기준선 1,294닢이
     안 움직이는 것 + `probe-purse.mjs`의 후반 천장이 실제로 걷히는 것, 둘 다).
     규칙은 `scenes/battle.js`(패배)와 `scenes/map.js`(도주)가 갖는다 — **둘이 이 한 벌을 함께 읽는다.**
     여기 값을 고칠 때는 그 두 파일을 다시 재야 한다. */
export const ENCOUNTER_LOSS = {
  /* ① 몫 — 지금 코드에 박혀 있던 그 숫자다(옮겨만 온 것이라 이 자체는 밸런스 변화가 아니다) */
  loseShare: 0.50,        // 패배: 금고의 이만큼 (`battle.js`)
  fleeShare: 0.12,        // 싸우기 전 도주: 금고의 이만큼 (`map.js`)
  fleeCargoShare: 0.35,   // 그때 넘기는 화물 몫 — `JETTISON_SHARE`(폭풍 투하)와 **다른 값**이다

  /* ② 상한 — 「적도 옮겨 실을 수 있는 만큼만」. `spoilsCap`/`capSpoils`의 거울이다.
     ★ `perCrew`가 이 설계의 심장이다: 상한이 **적의 크기**에 걸려야 내 금고와 무관해지고,
       그래야 `G* = 항차이익 / (조우율 × 손실률)`의 천장이 사라진다. */
  perCrew: 220,           // 적 갑판의 한 사람이 나를 수 있는 닢
  floor: 150,             // 아무리 작은 적도 이만큼은 가져간다 (초반이 안 바뀌게 낮게 둔다)
  /* ★ **B 확정(실측)** 0.10 → 0.03. 이 값이 천장을 정한다 — 상한 위 실효 손실률이
     `share × tail`이라 `G* = P/(q·share·tail)`이고 **`G*`는 `1/tail`에 비례**한다.
     0.10에서는 `G*`가 464,253닢에 그쳐 패권 총투자 912,630닢에 **못 닿았다**.
     ⚠️ 기준 P를 8,000이 아니라 **2,322**로 잡았다 — `probe-fleet 24판` 실측에서
       항차 순이익이 화물칸 320칸 위로 **평평해진다**(시장 깊이가 병목이라 선복을 늘려도
       채우는 칸이 132에서 안 는다). 후반 부자의 P는 크지 않다.
       0.05는 1.18배로 빠듯하고 0.03이 **1.97배** 여유다(거점 유지비 4,563닢/30일까지 얹힌다). */
  tail: 0.03,             // 상한 위로도 이만큼은 — 큰 놈에게 지는 것이 여전히 더 아프게
  /* 적 선원 수를 못 읽는 자리를 위한 폴백(도주는 적 정의가 얕게만 잡힐 수 있다).
     등급(1~5)에 이것을 곱해 갑판 인원을 어림잡는다. */
  crewPerLevel: 14,

  /* ③ 선원 바닥 — 위 「선원」 절의 직격. `battle.js`의 `Math.max(4, crew × 0.5)`를 대신한다 */
  crewShare: 0.50,        // 패배하면 남는 선원 비율 (지금 값 그대로)
  crewFloor: 4,           // 절대 바닥
  /* ★ 절대 바닥만으로는 모자란다 — 지금 하한 4가 원양 최소 3을 **간신히 스쳐서**,
     금고가 비면 사람을 못 태워 잠긴다(C-17과 같은 자리). 참이면 그 배의 원양 최소 인원
     (`state.js: OCEAN_CREW_MIN`으로 정해지는 그 수) 아래로는 안 내려간다. */
  crewFloorOcean: true,

  /* ④ 하한 보장 — 「다음 항차를 뜰 것은 남긴다」(응급 처치 · ②와 겹쳐 쓸 수 있다).
     남기는 것은 **여유가 아니라 한 항차**다 — 가장 짧은 항로의 항해비 + 최소 인원 고용비.
     끄고 싶으면 `keepAfloat: false`. */
  /* ★ **B 확정: 꺼 둔다.** 사용자 결정이 「상한 + 선원 바닥」(ⓐ+ⓓ+ⓒ)이고 **ⓑ 하한 보장은
     채택되지 않았다** — *"파산의 긴장은 남긴다"*(잠김은 화면 안내 C-17로 푼다).
     ⚠️ 그래서 규칙 쪽에 **배선하지 않았다** — 이것을 `true`로 돌려도 아무 일도 안 일어난다.
       되살리려면 `battle.js: finish('lose')`에 항해비 하한을 붙이는 일이 함께 필요하다. */
  keepAfloat: false,
  keepMul: 1.0,           // 그 「한 항차」에 곱하는 여유 (1.0 = 딱 한 항차)
};
