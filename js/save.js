...
const FILL_IF_MISSING = {
  ...
  dues: () => ({}),
  /* 초무한 자(`ROSTER` · 회차 24) — 옛 판은 **아무도 초무하지 않은 것으로** 이어진다.
     ⚠️ 읽는 쪽은 전부 `state.tamed?.[id]`라 없어도 안 깨지지만, 여기 적어 두면
     `resetGame`이 비우는 것과 이어한 판의 모양이 같아진다(`slain`과 같은 이유로 명시한다). */
  tamed: () => ({}),
  bountyDue: () => [],       // 아직 못 받은 현상금 — 옛 판은 받을 것이 없다
  /* ── 상단(商團) · 회차 25 ─────────────────────────────────
     ★ **안 넣어도 옛 판이 안 깨진다**(읽는 자리가 전부 `?.`·`||=`·`?? {}`로 열려 있고
       `guildTick`이 비면 `initGuilds()`를 부른다 — 회차 25 NPC-PM이 확인했다).
       그래도 적어 두는 이유는 **세이브 대상 목록이 곧 문서**이기 때문이다 —
       `dashboard/architecture.mjs`의 상태 트리와 이 표가 갈리면 다음 사람이 둘 다 못 믿는다.
     ⓘ `guilds`가 `{}`면 `initGuilds()`가 명부에서 새로 세운다(자본·호감이 첫인상으로 선다). */
  guilds: () => ({}),        // 상단 장부(cap·fleet·regard·voy) — 비면 명부에서 다시 세운다
  guildFlow: () => ({}),     // 상단이 민 순유입/순유출 — 비면 값이 안 밀린 상태로 이어진다
