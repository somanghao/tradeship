...
export const GOODS = [
  ...
  /* ── 수직계열화 1단계가 들여온 가공품 (A-9) ───────────────────
     ★ **base를 사료가 아니라 「가공마진 밴드」가 정했다.** 이것이 이 값의 정직한 출처다 —
       날염 사슬은 `면포 2 + 인디고 1 → 사라사 2`이고, 밴드(`data.js: WORK` 1.08~1.25)가
       요구하는 것은 `320×2 ÷ (145×2 + 260) = 640/550 = **1.164**`이다.
       320보다 낮으면 가공비 10%에 먹혀 염색장이 죽고, 높으면 *"시세로 사서 가공해도 남는"*
       구간이 열려 수직계열화가 새 수입원이 된다. → SPEC-vertical.md §2-3
     ★ 사료가 말해 주는 것은 **방향**이다 — 인도 날염포는 유럽에서 흰 면포의 서너 배에
       팔렸고 그것이 캘리코 열풍의 실체였다. 곡물 대비 16배(면포 7.25배의 2.2배)가
