...
export const OCEAN_LANES = [
  ...
  /* ★ 마젤란 해협 — 운하가 없는 세계에서 배가 남미를 넘는 **유일한 길**(2026-09-03 신설).
     지금까지 남아메리카는 **권역 안에서 두 조각**이었다. 대서양안 열다섯과 태평양안 열셋이
     권역 안 항로로 안 이어져 있어서, 브라질에 있는 배가 카야오로 가려면 **카리브를 거쳐**
     포르토벨로 노새길을 넘고 파나마에서 다시 배를 타야 했다(그 길은 이미 있다).
     ⇒ 패권 ①(그 권역 모든 항구에 거점)이 15/28에서 멈춘 자리가 이것이었다.

     ★ **파나마 운하는 1914년이다.** 이 시대에 남미를 넘는 길은 둘뿐이었고 둘 다 게임에 있어야 한다 —
       ① **지협 노새길**: 짧고 안전하되 짐을 내려 실어야 하고 **딴 권역(카리브)을 지난다.**
          스페인 은은 전부 이 길로 갔다.
       ② **마젤란 해협**: 배가 그대로 넘는 유일한 길. 대신 값이 지독하다.
          드레이크(1578)·판 노르트(1599)·스하우턴 같은 **침입자**만 이 길을 썼다.
     그 대비가 그대로 플레이어의 선택이 된다.

     ⚠️ **좌표로 못 잰다** — 산줄리안(248,212)에서 발디비아(104,214)로 직선을 그으면
       파타고니아를 관통한다. 실제 길은 지도 아래로 내려가 해협을 지나 칠레 해안을 거슬러
       오르는 길이다. 이 게임이 그런 구간을 다루는 방식이 **원양 항로**이고
       (`days`를 직접 적는다 · 위 `luanda~mocambique` 희망봉이 같은 모양이다 — **권역 안인데
       대륙 끝을 도는 길**), 그래서 여기에 적는다.
     ★ 원양 항로라 **동행 1척이 붙어야 들어간다**(`oceanReady().escort`). 사료와 맞는다 —
       혼자 들어간 배는 대개 안 나왔다. */
  { a: 'sanjulian', b: 'valdivia', days: 34, risk: 10.5, season: 'summer',
    note: '마젤란 해협. 남반구 여름이 아니면 편서풍과 얼음에 몇 달이 갇힌다. 항구도 사람도 없어 노리는 자조차 없지만, 그래서 무슨 일이 나도 아무도 오지 않는다.' },
  ...
  /* ── 주인선(朱印船)의 길 — 일본에서 남으로 내려가는 세 줄 ───────────────
     ★ 왜 여기에 선 셋을 더 긋는가: 동아시아에서 **밖으로 나가는 문이 둘뿐**이었고
       (광저우~믈라카 · 마닐라~아카풀코) 그 둘이 아홉 권역 중 최소였다. 게다가 둘 다
       대륙 남쪽 끝이라 부산포에서 스물여드레를 가야 문 앞에 선다 — 실클릭 여섯 회차가
       663일 동안 이 바다를 못 벗어난 구조적 원인이다(DESIGN-growth P7-2).
       **관문(`oceanReady`·`FLEET.escortAt`)은 한 줄도 안 낮춘다. 문을 늘린다.**

     ★ 기점을 나가사키로 잡은 것이 이 세 줄의 전부다. 부산포에서 나가사키가 **8일**이고
       광저우는 28일, 나하는 36일이다(하카타~히라도~나가사키 세 구간). 한반도에서 시작한
       사람이 **가장 먼저 닿는 문**이 나가사키라야 이 설계가 선다.

     사료: 1604~1635년에 발급된 슈인장(朱印狀) 356통의 목적지 분포가 이 세 줄의 근거다.
