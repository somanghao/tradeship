...
export const ROUTE_RISK = {
  ...
  /* ★ **이 바다의 유일한 뭍길**(요율 null · DESIGN-growth P7 곁작업).
     아프리카에는 육로·내해 구간이 하나도 없었다. `state.js: rollSeaEvent`는 `isInland`인
     구간에서만 노상강도·통행세를 내므로, 이 바다에서는 **사건 열아홉 중 둘이 영영 안 났다**
     (인도양도 같았다). 도시를 새로 만들지 않고 **이미 있는 두 항구 사이의 성격을 바로잡아**
     그 자리를 낸다 — 이 구간은 원래부터 바다가 아니었다.
     사료: 볼타 어귀에서 동쪽으로 아네호·위다·포르토노보·바다그리·라고스를 지나
     베냉 강까지 **모래톱 뒤의 라군과 크리크가 끊기지 않고 이어진다.** 큰 배는 사주가
     얕아 못 들어가고 카누만 지났으며, 위다의 우에다 왕과 라고스·바다그리의 수장들과
     베냉의 오바가 **길목마다 관세(customs)를 받았다.** 통행세와 노상강도가 나야 할 자리가
     여기다 — 그웨이토(우그호톤) 자체가 베냉 강 크리크를 거슬러 올라간 강항이다. */
  'gwato|whydah':    null,  // 라군·크리크의 카누길. 배가 아니라 짐과 사람이 뭍 안쪽으로 간다
