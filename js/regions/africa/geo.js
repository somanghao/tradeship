...
//   화풍(`STYLES`)도 latin/hellenic/levant 셋뿐이라 산호석 스와힐리 도시는 levant로,
//   포르투갈 요새는 latin으로 뒀다. `art.styleTodo` 참조.
...
export const CITIES = [
  ...
  { id: 'arguin',     name: '아르갱',   area: '사하라 해안',   style: 'levant', x: 44,  y: 32,  flag: 'spain',  seed: 4101, size: 1, industry: 1 },
  // 산티아구(히베이라 그란지) — 1462년 정착. 열대 최초의 유럽 상설 도시이고
  // 기니 해안으로 나가는 배가 모두 여기서 물과 면포를 싣는다.
  { id: 'santiago',   name: '산티아구', area: '카보베르데',    style: 'latin',  x: 22,  y: 62,  flag: 'spain',  seed: 4102, size: 2, industry: 1 },
  // 아심 — 앙코브라 강이 사금을 실어 내리는 자리. 1515년 산투 안토니우 요새.
  { id: 'axim',       name: '아심',     area: '황금해안',      style: 'latin',  x: 52,  y: 100, flag: 'spain',  seed: 4103, size: 1, industry: 1 },
  // 엘미나(상 조르즈 다 미나) — 1482년. **이 권역 전체의 이유**다.
  // 16세기 초 이 해안에서 해마다 금 680kg(24,000온스)이 나갔고 그것이 당시 세계 금 공급의 1/10이었다.
  { id: 'elmina',     name: '엘미나',   area: '황금해안',      style: 'latin',  x: 80,  y: 92,  flag: 'spain',  seed: 4104, size: 2, industry: 1 },
  // 그웨이토(우그호톤) — 베냉 왕국의 강어귀 외항. 도시 본체는 뭍 안쪽이라
  // 여기는 오바의 세관이 서는 자리에 가깝다. 유럽선은 여기까지만 들어왔다.
  { id: 'gwato',      name: '그웨이토', area: '베냉',          style: 'levant', x: 118, y: 100, flag: 'hafsid', seed: 4105, size: 2, industry: 1 },
  // 상투메 — 1493년 정착, 1515년 물레방아 제당소가 들어서면서 섬 전체가 사탕수수밭이 됐다.
  // 1595년 반란 때 불탄 것만 제당소 85곳 중 60곳. 먹을 것은 죄다 배로 들어온다.
  { id: 'saotome',    name: '상투메',   area: '기니만',        style: 'latin',  x: 124, y: 128, flag: 'spain',  seed: 4106, size: 2, industry: 1 },
  // 루안다 — 1576년. 콩고 왕국이 화폐로 쓰던 은질부 조개가 앞바다 섬에서 나고,
  // 카탕가의 구리 십자 주괴가 강을 타고 내려온다. 희망봉으로 나가는 서안의 마지막 큰 항구.
  { id: 'luanda',     name: '루안다',   area: '앙골라',        style: 'latin',  x: 80,  y: 156, flag: 'spain',  seed: 4107, size: 2, industry: 1 },
  // 벵겔라 — 뭍은 사막이고 앞바다는 차가운 벵겔라 해류다. 배후 고원의 상아가 유일한 밑천.
  { id: 'benguela',   name: '벵겔라',   area: '앙골라',        style: 'latin',  x: 66,  y: 180, flag: 'spain',  seed: 4108, size: 1, industry: 1 },
  ...
  { id: 'cabo',       name: '희망봉',   area: '아굴라스',      style: 'latin',  x: 146, y: 200, flag: 'spain',  seed: 4109, size: 1, industry: 0 },
  ...
  { id: 'sofala',     name: '소팔라',   area: '무타파',        style: 'levant', x: 228, y: 182, flag: 'spain',  seed: 4110, size: 1, industry: 0 },
  // 모잠비크 섬 — 희망봉을 돈 인도 항로 선단이 **처음 닻을 내리는 자리**다.
  // 생산지가 아니라 배를 고치고 겨울을 나는 곳이라 industry를 2로 뒀다.
  { id: 'mocambique', name: '모잠비크', area: '모잠비크',      style: 'latin',  x: 256, y: 158, flag: 'spain',  seed: 4111, size: 2, industry: 2, prizeYard: true },
  // 킬와 — 12세기에 소팔라의 금을 빼앗아 스와힐리 해안의 맹주가 된 술탄국.
  // 1505년 알메이다가 500명을 상륙시켜 무너뜨린 뒤로는 궁전만 남았다.
  { id: 'kilwa',      name: '킬와',     area: '스와힐리',      style: 'levant', x: 282, y: 130, flag: 'hafsid', seed: 4112, size: 2, industry: 1 },
  // 잔지바르 — 뭍의 물건과 바다 건너 물건이 임자를 바꾸는 중개상의 섬.
  // 포르투갈은 1503년에 조공만 받아 갔을 뿐 요새도 수비대도 두지 않았다(1591년 영국선 기록).
  { id: 'zanzibar',   name: '잔지바르', area: '스와힐리',      style: 'levant', x: 302, y: 112, flag: 'hafsid', seed: 4113, size: 2, industry: 1 },
  // 몸바사 — 이 해안 최대의 항구이자 가장 완강하게 저항한 도시. 1500·1505·1528·1589년에
  // 네 번 불탔고 그때마다 다시 섰다. 다우 건조 전통이 있어 industry 2.
  { id: 'mombasa',    name: '몸바사',   area: '스와힐리',      style: 'levant', x: 320, y: 90,  flag: 'hafsid', seed: 4114, size: 3, industry: 2 },
  // 말린디 — 몸바사의 숙적이라 포르투갈과 손을 잡았다. 1498년 다 가마를 환대한 도시로
  // 그때 인구 5,000~10,000. 유럽 배에 관세가 헐한 것이 이 도시의 성격이다.
  { id: 'malindi',    name: '말린디',   area: '스와힐리',      style: 'levant', x: 336, y: 70,  flag: 'hafsid', seed: 4115, size: 2, industry: 1 },
  // 라무·파테 군도 — 뱃집의 섬. 못 하나 안 쓰고 야자 노끈으로 판자를 꿰매는
  // 므템베 목수들이 여기 있었다. 1506년 포르투갈 봉쇄로 연 600메티칼의 조공을 물었다.
  { id: 'lamu',       name: '라무',     area: '스와힐리',      style: 'levant', x: 354, y: 54,  flag: 'hafsid', seed: 4116, size: 2, industry: 2 },
  // 모가디슈 — 이븐 바투타가 "지극히 큰 도시"라 적은 곳. 토브 베나디르라 불린 직물을
  // 이집트와 시리아로 내다 팔았다. 이 권역에서 유일하게 **만든 것을 수출하는** 항구다.
  { id: 'mogadishu',  name: '모가디슈', area: '베나디르',      style: 'levant', x: 364, y: 32,  flag: 'hafsid', seed: 4117, size: 3, industry: 1 },
];
