...
export const PIRATES = [
  ...
  /* ── 돈으로 산 함대 ────────────────────────────────────────
     ★ **이 자리는 오래 비어 있었다.** 아홉 바다 중 아프리카에만 두목(strength 5)이 없었는데,
       서아프리카에서 포르투갈 독점을 깬 것은 대개 **익명의 인터로퍼 함대**라 이름을 붙일 수
       없었기 때문이다. 미르 알리 베이를 올릴까 했으나 그는 갤리 한두 척짜리였다(위 항목 주석).
     ⇒ 사용자 설계로 푼다: **뱃사람이 아니라 왕이 두목인 함대.**

     기니만의 오바(oba). 금·상아·후추와 **사람**을 팔아 쌓은 재화로 유럽 뱃사람을 통째로
     사들였다 — 배를 몰고 포를 쏘는 것은 여러 나라에서 흘러든 삯꾼들이고, **갑판에 올라
     칼을 드는 것은 오바의 병사들**이다. 그래서 이 함대는 어느 나라 깃발도 달지 않는다.
     ★ **고증**: 베냉은 실제로 금·상아·후추로 부유했고 유럽과 직접 거래했으며 강력한
       상비군을 두었다(오바의 궁정과 청동 두상이 그 부의 증거다). 다만 **이 함대는 사료에 없다** —
       「부유한 서아프리카 왕국이 유럽 뱃사람을 고용해 함대를 굴렸다면」이라는 가정이고,
       그래서 근거 표시는 `gameplay`다. 알고 그렇게 뒀다.
     ★ `flag: 'benin'`은 이미 있는 키다(`js/sprites/ship.js: FLAGS`) — 이 자리를 위해
       만든 것이 아니라 **베냉이 이 바다에 이미 서 있었다**는 뜻이다.
     ★ `ship: 'galleon'`인 것이 이 함대의 성격이다 — 제 바다에서 지은 배가 아니라 **사 온 배**다. */
  {
    id: 'obafleet', name: '오바의 삯함대', flag: 'benin', ship: 'galleon',
    base: 'gwato', purse: [5200, 16500], strength: 5, bounty: [5200, 12000],
    hunt: ['elmina|gwato', 'gwato|saotome', 'accra|whydah', 'accra|elmina'],
    circuit: ['gwato', 'saotome', 'loango', 'saotome', 'gwato', 'elmina', 'accra', 'elmina', 'gwato'],
    season: null, scope: 'ocean',
    blurb: '왕이 돈으로 산 함대다. 배는 유럽 삯꾼이 몰고, 갑판에 오르는 것은 오바의 병사들이다.',
    lines: {
      hail: '“오바의 배다. 이 물에서 값을 정하는 것은 너희 왕이 아니다.”',
      spare: '“짐은 두고 가라. 사람은 안 가져간다 — 우리가 파는 쪽이지 사는 쪽이 아니다.”',
    },
  },
