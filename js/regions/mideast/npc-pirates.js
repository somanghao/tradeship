...
export const PIRATES = [
  ...
  /* ── 홍해에서 인도양으로 내려간 함대 ────────────────────────
     하디임 술레이만 파샤. 1538년 이집트 총독으로서 **갤리 일흔 척 남짓**을 몰고 수에즈를
     떠나 아덴을 접수하고 디우를 포위했다. 이 바다가 낸 가장 큰 함대이고, 오스만이
     인도양을 실제로 넘본 순간이다(포위는 실패했고 함대는 되돌아왔다).
     ★ 이 바다의 유일한 strength 5다 — 셀만 레이스(4)·피리 레이스(4)가 그 앞뒤에 선
       인물들이라 **그 위에 설 자리가 비어 있었다.** 아홉 바다 중 두목이 없던 셋 가운데 하나다.
     ★ 소굴이 수에즈인 것은 사료 그대로다. 회차 29에 이 바다의 t3(`kalyon`)를 넣은 자리도
       거기다 — 오스만 홍해 함대의 조선지가 곧 이 함대의 출항지였다.
     ★ 사냥터를 **아덴만 남쪽**으로 둔 것은 셀만 레이스(지다~모카)와 겹치지 않게 하려는 것이고,
       사료로도 이 함대가 실제로 지난 물이다(아덴 접수 → 소코트라 앞 → 인도양). */
  {
    id: 'hadimsuleiman', name: '하디임 술레이만 파샤', flag: 'ottoman', ship: 'kalyon',
    base: 'suez', purse: [5000, 16000], strength: 5, bounty: [5200, 12000],
    hunt: ['aden|mokha', 'aden|shihr', 'aden|socotra', 'shihr|socotra'],
    circuit: ['suez', 'tur', 'qusayr', 'jeddah', 'luhayya', 'mokha', 'aden', 'socotra',
      'aden', 'mokha', 'luhayya', 'jeddah', 'qusayr', 'tur', 'suez'],
    season: 'summer', scope: 'ocean',
    blurb: '술탄의 함대다. 수에즈에서 내려와 아덴을 접수하고 인도까지 갔다 — 한 번은 되돌아왔다.',
    lines: {
      hail: '“이 바다는 술탄의 것이고, 그 아래 바다도 곧 그리 될 것이다.”',
      spare: '“함대는 상선을 세지 않는다. 길만 비켜라.”',
    },
  },
];
