// regions/mediterranean/geo.js — 지중해의 지리
//
// ★ 이 파일은 "어디에 무엇이 있고 어떻게 이어지는가"만 다룬다.
//   교역품 산지·수요지 같은 **경제 수치는 `trade.js`**에 있다. 둘을 갈라 둔 이유는
//   지도를 손보는 사람과 경제를 조율하는 사람이 같은 줄을 놓고 충돌하지 않게 하기 위해서다.
//   두 쪽은 `id`로만 맞물린다.
//
// 좌표계: 논리 해상도 400×225 기준이고 **이 권역 안에서만 의미가 있다.**
//   다른 권역의 (141,63)과는 아무 관계가 없다. 지도 그림(`js/sprites/scene.js: mapSprite`)과
//   같은 좌표계이므로, 해안선을 고치면 도시 좌표도 함께 봐야 한다.

/** 도시의 지리·외형.
    x,y   지도 위 위치(400×225 · 이 권역 안에서만 유효)
    area  소지역명 — 화면에 도시 이름 옆에 붙는 표시용 라벨(권역과 다르다)
    style 항구 배경 화풍 — sprites/scene.js: STYLES
...
export const CITIES = [
  { id: 'venezia',    name: '베네치아',     area: '아드리아',   style: 'latin',    x: 141, y: 63,  flag: 'venice',  seed: 1101, size: 3, industry: 3 },
  { id: 'genova',     name: '제노바',       area: '리구리아',   style: 'latin',    x: 116, y: 76,  flag: 'genoa',   seed: 1202, size: 3, industry: 3 },
  { id: 'marseille',  name: '마르세유',     area: '프로방스',   style: 'latin',    x: 91,  y: 71,  flag: 'france',  seed: 1303, size: 2, industry: 2 },
  { id: 'barcelona',  name: '바르셀로나',   area: '카탈루냐',   style: 'latin',    x: 57,  y: 89,  flag: 'spain',   seed: 1404, size: 3, industry: 2 },
  { id: 'napoli',     name: '나폴리',       area: '캄파니아',   style: 'latin',    x: 131, y: 110, flag: 'spain',   seed: 1505, size: 2, industry: 2 },
  { id: 'palermo',    name: '팔레르모',     area: '시칠리아',   style: 'latin',    x: 151, y: 144, flag: 'spain',   seed: 1606, size: 2, industry: 2 },
  { id: 'tunis',      name: '튀니스',       area: '이프리키야', style: 'levant',   x: 174, y: 157, flag: 'hafsid',  seed: 1707, size: 2, industry: 1, prizeYard: true },
  { id: 'algiers',    name: '알제',         area: '마그레브',   style: 'levant',   x: 106, y: 151, flag: 'ottoman', seed: 1808, size: 2, industry: 1, prizeYard: true },
  { id: 'athens',     name: '아테네',       area: '아티카',     style: 'hellenic', x: 191, y: 110, flag: 'ottoman', seed: 1909, size: 2, industry: 1 },
  ...
  { id: 'rodos',      name: '로도스',       area: '에게',       style: 'hellenic', x: 239, y: 109, flag: 'hospitaller', seed: 2010, size: 1, industry: 1 },
  { id: 'istanbul',   name: '이스탄불',     area: '보스포루스', style: 'hellenic', x: 241, y: 55,  flag: 'ottoman', seed: 2111, size: 3, industry: 3 },
  ...
  { id: 'bursa',      name: '부르사',       area: '비티니아',   style: 'hellenic', x: 222, y: 66,  flag: 'ottoman', seed: 2414, size: 2, industry: 0 },
  { id: 'iznik',      name: '이즈니크',     area: '비티니아',   style: 'hellenic', x: 242, y: 76,  flag: 'ottoman', seed: 2515, size: 1, industry: 0 },
  ...
  // 키오스 — 제노바 마오나가 쥐고 있던 섬. **마스틱(유향나무 수지)의 사실상 유일한 산지**이고
...
/* 항로 — 인접 도시 간 연결. 여기에 없는 두 항구는 직항이 없다.
   ...
   ★ 다른 권역으로 나가는 선은 여기가 아니라 `js/regions/index.js: OCEAN_LANES`에 적는다. */
export const ROUTES = [
...
/* 항로 위험도 — **당대 해상보험 요율(%)**이다.
   ...
   판정과 출처는 `content/regions/mediterranean-evidence.json`이 정본이고
