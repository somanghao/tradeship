...
//
// ★ 왜 손으로 안 쓰고 생성하는가.
//   지도 외주를 세 번 반려했고, 1·2차의 원인이 **의뢰서와 게임 데이터가 어긋난 것**이었다.
//   1차는 좌표표를 사람이 옮겨 적다 틀렸고, 2차는 그 사이에 도시가 늘었는데 의뢰서가 낡았다.
//   3차(AI 생성)는 의뢰서가 맞았는데 **AI가 정사각형으로 뽑아 비율부터 어긋났다** — 그 절이 §1-A다.
...
import { MAPS, CLIMATE } from '../js/sprites/maps/index.js';
...
/* 공통 절 — 아홉 장이 똑같이 이고 갈 규칙. 여기가 이 의뢰서의 심장이다.
   1·2·3차 반려 사유가 전부 이 절을 안 지켜서 생겼다. */
const COMMON = `
...
function briefFor(rg) {
  ...
  const CLIMATE_NOTE = CLIMATE_KO[climate] ?? '';
