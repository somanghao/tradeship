...
export const STATE_FIELDS = [
  ...
  { k: 'royal', g: '진행', d: '**작위(爵位)와 개항**(§A-11 조선) — `{taken, tier, opened, fair}`. 사용자 원문 *"한반도 왕에게 미션을 받고 … 개항하고 작위를 받아서 한반도 전체 공업력을 올리고"*. **판에 하나뿐인 축**이다(세력별 `regard`·항구별 `crewRep`·깃발별 `infamy` 셋 다 다른 축이고 전부 벌점 방향이라 어느 것의 꼭대기도 아니다). 계단 셋(`data.js: ROYAL.steps` · 아홉 바다 중 3·5·7)이 저마다 다른 것을 연다 — ①통정대부=**개항**(바깥 배 넷이 조선 항구를 돌기 시작하고 세 −15%) ②가선대부=**천장**(`civicCapOf` 3→4 · ★ `CIVIC.dues[3]`·`days[3]`이 이미 있는데 죽어 있던 값을 살린다) ③첨지중추부사=**문턱**(`civicDuesOf` −25%). ⛔ **금화·수입을 한 닢도 안 준다**(`HEGEMONY` 주석의 방어선) · ⛔ **조정을 `FACTIONS`에 안 넣는다**(§A-11 ㄴ — `check-factions`가 `busanpo: null`을 잠갔고 소설 28·64·68장이 걸려 있다 · 왕의 뜻은 **인물**이 전한다). `fair`는 **개시(開市)** — 상시 개방이 아니라 `ROYAL.fairEveryDays`마다 서는 **달력**이고(사료가 「1년 몇 회」 꼴이다) `rollShockEvents` 안에서 열려 `advanceDays`/`waitDays` **두 입구 문제**가 안 생긴다' },
