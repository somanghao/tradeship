...
| 발주용 기준판 | `assets/map-reference/` — `coord-target.png`(도시 정위치 1px) · `coord-target-4x.png`(판독용) · `rejected-overlay.png`(반려 근거) |
...
- **★ 기계 검수는 최소조건이지 합격조건이 아니다 — 2차 납품이 그 증거다.** 2차는 무손실(png와 픽셀 100% 동일)·좌표 정합·육지 비율까지 `check-map.py` 전 항목을 통과했는데, 눈으로 보면 지중해가 아니었다: 이탈리아가 삼각 쐐기, 그리스가 역삼각형, 해안선이 전부 직선 사선, 섬은 타원 얼룩, 고유색 **8색**(1차 PNG는 48색). 사실상 **도시·항로 그래프의 볼록껍질을 바다로 칠한 그림**이다 — 검사 항목만 역설계한 것이라 의뢰서 §4(반도·섬 실루엣)를 만족하지 않는다.
  - **원인은 또 의뢰서였다.** §0-A에서 "좌표를 먼저 놓고 지형을 맞춰라"고 했더니 그림쟁이가 **지형을 포기하는 방향**으로 맞췄다. 빠진 것은 **좌표를 이미 만족하는 실루엣 기준판** — 게임이 코드로 그리는 `SEA_SPANS` 지도가 정확히 그것인데 기준판 3종에 안 들어 있다. 다음 수는 그것을 4배로 뽑아 주고 "이 실루엣에 질감·명암만 얹어라"로 재의뢰하는 것.
  - `check-map.py`에 **색수 하한(≥24색)과 해안선 직선 구간 길이 상한**을 넣어 이 우회를 막을 수 있다(미구현). 부수 버그 — 상대경로 인자를 주면 `relative_to()`에서 크래시한다(`line 149`).
...
- **그림을 외부(사람·AI)에 발주할 땐 의뢰서가 정본이다.** `assets/PORT-BACKGROUND-BRIEF.md`(항구 16장) · `assets/WORLD-MAP-BRIEF.md`(지도 1장)에 규격·수평선/흘수선/부두 y좌표·UI 가림 영역·팔레트·검수기준이 다 들어 있다. **생성형 AI는 진짜 픽셀 그리드를 못 만들므로** 크게 생성 → 최근접이웃 축소 → 양자화 → 1px 손보정 후처리가 필수다(의뢰서에 명시).
