...
| `js/npc/houses.js` (+`-euro`/`-asia`/`-newworld`) | **상단(商團) 명부 53곳** — 배가 아니라 **회사**다(지도에 안 뜬다) | `HOUSES`, `HOUSE_BY_ID`, `housesOfRegion`, `housesAtCity` |
| `js/npc/guild.js` | **상단 엔진** — 여러 칸을 보고 무역로로 부를 쌓는다. 그 부가 **가격차를 좁힌다**(`state.guildFlow`) · 자본→선단·세기 사다리 · 세 갈래(괴롭힘·이용·도움) | `guildTick`, `initGuilds`, `guildRank`, `guildsAtCity`, `guildRegard`, `guildFoeOnLeg`, `guildOfferAt`, `settleGuildOffer`, `guildPriceReport`, `houseLanes` |
...
| `js/payday.js` | **급여 정산 화면**(월말 결산서) — 장부·선창·다음 밑천·갑판 분위기 → [payroll.md](payroll.md) | `openPayday(onDone)` · 항구 씬이 입항 때 부른다 |
| `tools/test-tavern.mjs` | 술집·시작 조건 검증 20종 | 시작(선원 0·금화 200) · 결정론 · 항구 성격 · 기질별 값 · **첫 항차 성립** · 명부 동기화 |
| `tools/test-payroll.mjs` | 급여 정산·체불·이탈 검증 | 발생주의(급여만 쌓임) · 정산 · 참을성별 불만 · **화물 절도** · **장부↔금고 정합** · 밀린 달 처리 |
...
port ──출항──▶ map ──도시 클릭──▶ (항해 연출)
     ...
     └ 입항 시 급여일이 지났으면 **정산 모달**(payday.js) — 닫을 수 없다
                     │
                     ├ 이벤트 없음 ─▶ port (도착)
                     ├ 폭풍/표류물 ─▶ 모달 ─▶ 항해 재개
                     └ 해적 조우 ──▶ battle
