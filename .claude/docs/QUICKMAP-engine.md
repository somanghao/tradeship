...
- **스프라이트 캐시는 6MB LRU 상한이다**(`pixel.js: CACHE_MAX_BYTES`). 상한을 넘으면 **가장 오래 안 쓴 것부터** 버리고, 다시 필요하면 그때 굽는다. 매 프레임 쓰이는 스프라이트는 항상 최근이라 안 버려진다. 실측: 상한이 없을 때 항구 16곳+선박 전조합이 **12.5MB**까지 갔고, 지금은 5.8MB에서 멈춘다. 진단은 `cacheStats()`.
