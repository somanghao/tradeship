...
- **브라우저 스크린샷이 CDP 타임아웃** — 캔버스가 많은 페이지에서 `Page.captureScreenshot`이 30초로 실패한다. **렌더러가 죽은 게 아니다.** 그대로 한 번 더 호출하면 대개 성공한다 — 페이지를 리로드하거나 코드를 의심하기 전에 재시도부터. 스크롤 직후 검은 화면이 찍히면 1틱 스크롤로 리페인트를 유도한다. (2026-08-26 세션필독 다이어트로 gotchas #3에서 이관)
...
- **패널이 덮는 쪽에는 그림을 그리지 마라.** 조선소는 x≥206, 술집은 x≥196이 DOM 패널이다. 술집 배경을 400폭 전체에 그렸다가 카운터·창·그물이 통째로 가려져 왼쪽이 휑했다 — **그릴 수 있는 폭을 먼저 정하고** 그 안에 담는다.
- **인물 앞에 와야 하는 가구는 전경 스프라이트로 뺀다.** 배경 한 장에 테이블을 그리면 인물이 항상 위에 그려져 테이블이 발치에 깔린 널빤지가 된다. 술집이 `tavernSprite`(배경) + 인물 + `tavernFrontSprite`(전경) 3겹인 이유다.
- **씬 전환 시 남은 `setTimeout` 체인** — 전투 턴이 타이머 체인이라 씬을 급히 벗어나면 콜백이 살아 있다. 각 콜백이 `if (!B) return`으로 방어 중이니 이 가드를 지울 것.
- **스프라이트 캐시는 6MB LRU 상한이다**(`pixel.js: CACHE_MAX_BYTES`). 상한을 넘으면 **가장 오래 안 쓴 것부터** 버리고, 다시 필요하면 그때 굽는다. 매 프레임 쓰이는 스프라이트는 항상 최근이라 안 버려진다. 실측(항구가 16곳이던 시절): 상한이 없을 때 항구 전체+선박 전조합이 **12.5MB**까지 갔고, 지금은 5.8MB에서 멈춘다. 항구가 264곳이 된 뒤에도 상한이 있으니 값이 아니라 **버리는 순서**만 바뀐다. 진단은 `cacheStats()`.
- **전경·오버레이 스프라이트는 화면 크기로 굽지 마라 — 그리는 영역만 굽는다.** 술집 테이블 전경을 400×225로 구웠더니 **95%가 투명 픽셀**인 채 항구 배경 한 장과 같은 352KB를 먹었다(지금은 200×34 = 26KB). 화면 좌표를 그대로 쓸 수 있어 편하다는 이유로 이렇게 되기 쉽다 — 오프셋 상수를 두고(`TAV_FRONT`) blit 위치로 옮긴다.
- **정수배 스케일 정책** 때문에 창이 작으면 `scale=1`로 떨어진다(1.9배를 못 씀). 버그가 아니라 픽셀 선명도를 지키는 의도된 동작.
- **로그로 사건을 관측할 땐 `state.log`가 앞에 쌓인다는 것을 먼저 안다**(`pushLog`가 unshift · 60줄 상한). `slice(이전 길이)`는 *가장 오래된* 줄을 준다 — 이 하나로 아홉 바다가 전부 "사건 0건"이라는 거짓 결과를 냈다. → [playtest-harness.md](wiki/playtest-harness.md) §3
- **원양 항로는 지도에 선으로 안 그려진다**(권역마다 좌표계가 따로라 그을 좌표가 없다 · `map.js: oceanRows`). 캔버스 좌표 클릭으로는 못 가고 **사이드 `.route-row` 카드**로 간다 — 하네스도 그 길을 알아야 한다(`sail(id, {name})`).
  - ⚠️ **2026-08-24 정정 — `cityScreenPos`는 남의 바다 도시에도 좌표를 준다.** "좌표가 없어서 못 간다"가 아니었다. 이 함수가 **권역을 안 보고** `toScreen`을 돌려 광저우 지도에서 `melaka`에 `{x:444,y:296}`을 줬고, `sail`이 그 자리를 눌러 **마카오로 떠났다.** 하네스는 그것을 "미도착"으로만 읽어 **한 판 완주 회차 셋**을 잃었다. `js/main.js`가 이제 다른 권역이면 `null`을 준다. → [playtest-harness.md](wiki/playtest-harness.md) §3 · `.playtest/grand-run/FINDINGS-grand.md` G-F7
- **대양은 사람과 배가 성해야 건넌다** — `oceanReady()`가 **선체 25% 미만 · 선원 최소 인원의 60% 미만**이면 원양을 막는다(근해는 열린다 — 항구에 갇히지 않게 한 설계). 사냥 뒤 그대로 대양에 나가면 걸린다. **막힌 것을 "미도착"으로 읽지 마라** — 게임은 이유를 말한다.
- **확률이 낮은 사건은 표본을 설계하고 판정한다.** `INLAND_ODDS` 0.12면 두 번 타서 안 나는 게 정상(77%)이다. "안 났다"를 결함으로 적기 전에 몇 번 굴렸는지를 본다.
- **사람처럼 눌러 보는 검증은 OS 마우스가 아니라 CDP 입력 + 화면 커서 오버레이로 한다.** 이 PC에서 `SetCursorPos`/`mouse_event`는 좌표만 바뀌고 페이지에 `mousemove`가 닿지 않았다(`WindowFromPoint`가 늘 `Program Manager`). 창을 항상-위로 점유할 필요도 없다 — 오버레이는 페이지 안에서 그린다. → [playtest-harness.md](wiki/playtest-harness.md) §3
- **긴 시장 목록에서 좌표로 연속 클릭하지 마라.** 매매 후 배지·줄바꿈이 바뀌며 행 높이가 달라져 다음 클릭이 다른 품목을 누른다(실제로 도자기 3연타 중 1회를 흘렸다).
