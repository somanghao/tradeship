...
- ★ **서버가 남의 판을 서빙하고 있을 수 있다 — 파일이 다르면 코드가 아니라 배관을 의심한다.**
  워크트리에서 일하는 개발자가 여럿이면 **같은 포트를 두 `serve.py`가 노린다.** 뒤에 뜬 쪽은
  바인딩 실패로 **조용히 죽고**, 브라우저는 먼저 뜬 쪽에서 200을 받는다 — 곧 **남의 워크트리 코드**다.
  그러면 `gen-map-png.mjs` 같은 「띄워서 굽는」 도구가 **옛 코드로 산출물을 만든다**(실제로 지도 회차가
  30분을 태웠다: 바다 48% → 31.6%, 흑해 실종. 제 코드를 의심하며 두 번 되돌렸다).
  - **잡는 법**: `curl -s http://localhost:<포트>/js/… | wc -c`를 디스크의 `wc -c`와 비교한다.
    **바이트가 다르면 서빙되는 것이 내 파일이 아니다.**
  - **막는 법**: 포트를 남과 겹치지 않게 잡고, `serve.py`의 `directory=`를 **절대경로**로 박는다.
  - ⚠️ 이것은 gotchas #6(낡은 모듈 캐시)과 **증상이 같고 원인이 다르다** — 그쪽은 브라우저 캐시,
    이쪽은 **서버가 다른 디렉터리**다. 캐시버스터를 아무리 붙여도 안 낫는다.
