...
- **거점은 자산이면서 고정비다**(연 6%) — 못 내면 **한 번 문을 닫고 두 번째에 압류**. 임차창고는 **그 항구 시세를 열어 준다**(`priceTip`). ⚠️ 소유·일함·들른 곳·소문이 다 다르다 → `QUICKMAP-trade.md` §3
...
- **동료는 코멘다다**(`COMMENDA`) — **초반엔 자본이고 후반엔 비용**이다. 동행선에는 선장이 있어야 하므로 **선단 규모가 동료 수에 묶인다** → `wiki/world-npc.md`
...
- ★ **규칙이 멀쩡한데 화면이 말하지 않아 수백 일을 잃는다** — 한 회차에 다섯 번. **밸런스를 의심하기 전에 「화면이 그 사실을 말하나」를 먼저 본다.**
...
- **사람은 이길 수 있는 상대만 싸운다 — 전투 설계·측정의 전제**(`state.js: fleeOdds` → `QUICKMAP-combat.md`).
- ⚠️ **사다리 첫 칸이 낮다** — 지역 배가 싸져 첫 배가 1항차다(`UNIMPLEMENTED.md` C-2). 조정 후보.
- ⓘ **기함이 안 가라앉는 것은 설계다**(`state.js: spreadDamage`). 결함으로 적지 말 것.
