...
도메인: **art**(에셋·지형·배경) / **trade**(시세·항해비·급여·가공) / **world**(NPC·명부·세력·고증·대시보드) / **combat**(전투·선박·조선소) / **engine**(캔버스·씬·UI·실행) / **story**(소설 — 코드 아님)
...
- 게임 루프 3파트(무역·항해·전투) + 조선소·술집 · **후반 4종**(거점·공업력 승급·정박·**가공장** — 원료를 사 완제품으로 파는 `state.works`). **끝이 둘** — 조선의 「여덟 항구와 한 척」(`ENDING`)과 아홉 바다 **패권**(`HEGEMONY`). 아홉 바다는 **권역마다 자동 완주로 검증됐다**(A-8). **한 판으로 전부**는 동아시아까지 갔다(A-8e · 러너 `.playtest/grand-run/`) — 수치 정본은 `UNIMPLEMENTED.md`.
- **공개 저장소** `github.com/somanghao/tradeship` (public, main).
- 에셋은 대부분 코드 생성이고 PNG로 갈아 끼운다(`bake` 키 → `assets/manifest.json`). ⚠️ **manifest 키는 코드가 *부르는* 키여야 한다** → `QUICKMAP-art.md` §3.
- **지도 아홉 장은 PNG다**(`assets/map/` · 코드 생성은 폴백 · 사람이 그린 것은 반려 중 F-1). **좌표를 옮기면 `gen-map-png.mjs`를 다시 돌린다**(안 돌리면 `check-map.py`가 잡는다). ⚠️ **아홉 장만 미커밋** — 워킹트리판은 육지가 42px 주기로 반복된다(F-8). 고친 뒤 따로 커밋한다.
...
- **데이터는 세 겹 — UI ▸ 수치 ▸ 근거.** 권역 `geo.js` ▸ `trade.js` ▸ `content/regions/<권역>-evidence.json`. **수치를 고치면 근거도 같은 커밋에서**(`check-*`가 실패시킨다. *근거 없음*은 경고일 뿐). ★ 특산품·깃발은 이미 사료에 맞다 — **밸런스나 연표만 보고 되돌리면 같은 오류가 재발한다.**
- **한반도 갈래 다섯 중 하나로 시작한다**(`data.js: ORIGINS` · 부산포·마포·여수). 다른 여덟 바다는 `START_PORTS`. ★ **신분 특전은 조선 항구에서만 돈다**(`joseonOnly`) — 제1해에서 가장 센 갈래가 제3해에서 가장 약해진다.
- **선원은 술집에서 무리 단위로.** 값이 두 갈래라(계약금은 지금, 일당은 내내) 싸게 태운 대가가 나중에 온다. → `wiki/crew-tavern.md`
- **배는 도시 공업력이 정한다**(`industry ≥ SHIPS[].tier`). 상급선은 선행 선종을 몰아 봐야 열리고 즉시 얻는 길은 **중고선**뿐. ★ **`yardsOnly`인 배는 그 부두에서만**(철갑 거북선=염포 · 중고로도 안 나온다). 전투는 **탄종 선택** + 백병전 나포. → `QUICKMAP-combat.md`
- **배는 아홉까지 함께 몬다**(기함 + 동행 8 · `state.consorts`) — 선단은 **가장 느린 배**에 맞고 배마다 선원·항해비가 따로 든다. → `QUICKMAP-combat.md`
- **부관은 에이미 하나** — 첫날부터 탄 동행이라 계약금도 해고도 없다. → `wiki/officer.md`
- **세계가 혼자 돈다** — NPC 거래가 시세에 압력으로 남고, 해적은 턴 만큼 부유해진다. **항구마다 임자가 있어**(`FACTIONS`·`state.regard`) 덮치면 관계가 상한다. → `wiki/world-npc.md` · `QUICKMAP-world.md`
- **항로마다 위험이 다르고, 길수록 잦다.** 근거는 **당대 해상보험 요율**(`ROUTE_RISK`) + 그 구간 해적 수(판정 방식은 `QUICKMAP-world.md`). 내해·육로는 **노상강도·통행세**(`INLAND_ODDS`).
- **바다마다 적의 얼굴이 다르다** — 권역 `npc-pirates.js`의 `PIRATES`(**명부 40 · 꺾거나 초무하면 영영 안 나온다**)·`FOES`(티어 1~5). **수치는 `ENEMIES` 등급 그대로**다. → `QUICKMAP-world.md`·`QUICKMAP-combat.md` §3
...
- **물가·임금은 사료 비율에 맞춘다**(대조 축은 **곡물의 몇 배**·**선원 연봉으로 몇 개**). **‘닢’은 실화폐가 아니다** — 절대액 환산은 안 한다.
...
- **원양 관문은 배후지를 상대한다** — 대양을 잇는 항구(`OCEAN_LANES` 양끝)는 시장 깊이가 `MARKET.gateDepth`배다. 왕복 48~66일이라 **대량거래 벌점을 피할 수 없기 때문**이다.
- **큰돈은 확률이 아니라 사건에서 나온다** — `state.shocks`가 시세를 올리거나 **내린다**. 빈도가 도시 수를 따라가 바다를 넓혀도 밀도가 같다(`SHOCK.densityBase`). → `wiki/economy-trade.md`
- **눌러 보는 검증** — 사건 발동까지 보고 창은 **2번 모니터에만** → `wiki/playtest-harness.md`
- **배속은 연출만 줄인다**(`js/speed.js` · `?speed=8` · 화면 토글). 일수·확률은 배속과 **무관**하다. → `QUICKMAP-engine.md`
...
- **보험은 실제로 보상한다**(공동해손 손실의 30%). → `wiki/economy-trade.md`
- **`story/`에 이 세계를 무대로 한 장편소설이 있다.** **게임 데이터를 바꾸지 않는 것이 규약**이고,
  그 검수가 **게임 후반의 사양서**(A-1·A-1b·A-2)를 값까지 확정해 냈다 — 전부 게임에 들어왔다.
  → `story/GAME-LINK.md` §8 · 원고는 1장뿐이고 집필 순서·분량은 `story/OUTLINE.md` §집필 순서.
...
- 코드 쪽은 **지도 질감**(F-8 — ⚠️ 그대로 커밋하면 반복 무늬가 들어간다) · **기함 침몰·파산이 없다**(C-13·N4 — 동행선만 격침된다).
