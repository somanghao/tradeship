...
- **경제는 "여러 항차"와 "대형 주문 한 건"이 나란하다 — 그 한 건의 크기는 *선복*이 정한다**(보수는 기함 화물칸, 선금 천장은 선단 매각가 = 해상대차의 담보). 곡선은 `node tools/sim-stat.mjs 20`(분포)이고 **거기에 계약은 없다** — 계약 크기는 `node tools/sim-contract.mjs`, 선단이 시장 깊이를 뚫는지는 `node tools/sim-fleet.mjs`. **1회 실행으로 판단하지 않는다.** → `wiki/economy-trade.md`
