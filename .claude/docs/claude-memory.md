...
- **통합 대시보드** `/dashboard/` — 게임 모듈을 그대로 돌려 계측한다. **파일이나 state 필드를 추가하면 `dashboard/architecture.mjs`에 적어야** `check-architecture`가 통과한다.
...
- **입항세는 두 겹** — 기본율(`TARIFF`) + 도시 오버라이드(`CITY_TARIFF`). **비어 있는 도시는 빠진 게 아니라** 기본율로 구른다. 항구 성질은 `baseTariff()`로 읽는다.
- **데이터는 세 겹 — UI ▸ 수치 ▸ 근거.** 권역 `geo.js` ▸ `trade.js` ▸ `content/regions/<권역>-evidence.json`. **수치를 고치면 근거도 같은 커밋에서**(`check-*`가 실패시킨다). ★ 특산품·깃발은 이미 사료에 맞다 — **밸런스나 연표만 보고 되돌리면 같은 오류가 재발한다** → `QUICKMAP-world.md` §3
- **한반도 갈래 다섯 중 하나로 시작한다**(`ORIGINS` · 부산포·마포·여수 · 다른 여덟 바다는 `START_PORTS`). ★ **신분 특전은 조선 항구에서만 돈다**(`joseonOnly`) — 제1해에서 가장 센 갈래가 제3해에서 가장 약해진다.
...
- **세계가 혼자 돈다** — NPC 거래가 시세에 압력으로 남는다. **항구마다 임자가 있어**(`FACTIONS`·`state.regard`) 덮치면 관계가 상한다. → `QUICKMAP-world.md`
- **항로마다 위험이 다르고, 길수록 잦다.** 근거는 **당대 해상보험 요율**(`ROUTE_RISK`) — 내해·육로는 따로 문다 → `QUICKMAP-world.md`
- **바다마다 적의 얼굴이 다르다** — 권역 `npc-pirates.js`의 `PIRATES`(**명부 40**)·`FOES`. **수치는 `ENEMIES` 등급 그대로**이고 이름·국적·선체만 갈아 끼운다. → `QUICKMAP-world.md`
- **값 차이는 구조이고 흔들림은 시황이다** — `priceOf`가 공통 시황과 도시 사정을 갈라 곱한다. 도시마다 따로 흔들면 산지→수요 사다리가 뒤집힌다(광산에서 멀수록 은이 싸졌다).
- **경제는 "여러 항차"와 "대형 주문 한 건"이 나란하다 — 그 한 건의 크기는 *선복*이 정한다**(보수는 기함 화물칸, 선금 천장은 선단 매각가 = 해상대차의 담보). 곡선은 `node tools/sim-stat.mjs 20`(분포)이고 **거기에 계약은 없다**(크기는 `sim-contract.mjs`). **1회 실행으로 판단하지 않는다.** → `wiki/economy-trade.md`
...
- **거점은 자산이면서 고정비다**(연 6%) — 못 내면 **한 번 문을 닫고 두 번째에 압류**, 되팔면 40%. ⚠️ 소유(`ownsHolding`)와 일함(`hasHolding`)이 갈린다 → `QUICKMAP-trade.md` §3
- **삭은 배는 막히는 게 아니라 값을 문다** — 선체가 바닥이면 **느려지고 실은 짐이 젖는다**(`HULL`). 막으면 삭은 배로 3일 항로뿐인 항구에 갇힌다 → `QUICKMAP-engine.md` §3
- **명부 해적은 찾아가야 만난다** — 소식을 사면(`bounty-tip`) 그자의 사냥터에서 **그자가 온다**. 못 이길 상대는 소굴에서 **초무**(격파보다 비싸다). 조우 확률은 안 건드린다 → `QUICKMAP-world.md`
- **항해비는 일곱 갈래** — **성장할수록만 무거워진다.** 정박 중에도 나간다(`waitDays`)
- **값나가는 짐은 두 번 대가를 치른다** — 보험료와 해적 조우 확률이 함께 오른다(`cargoLure`).
- **원양 관문은 배후지를 상대한다** — 대양을 잇는 항구는 시장 깊이가 `MARKET.gateDepth`배(왕복이 길어 벌점을 피할 수 없다).
...
- **한글 조사는 `js/josa.js`(leaf)** — `ui.js`가 re-export. `을(를)`을 손으로 적지 않는다.
- **보험은 실제로 보상한다**(공동해손 손실의 30%) → `wiki/economy-trade.md`
- **`story/`에 이 세계를 무대로 한 장편소설이 있다** — **게임 데이터를 바꾸지 않는 것이 규약**이고, 그 검수가 후반 사양(A-1·A-1b·A-2)을 값까지 냈다. → `story/GAME-LINK.md` §8 · `story/OUTLINE.md` §집필 순서
...
- 코드 쪽은 **지도 질감**(F-8 — ⚠️ 그대로 커밋하면 반복 무늬가 들어간다) · **기함이 침몰하지 않는다**(C-13 — 동행선만 격침된다).
