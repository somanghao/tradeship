...
- 게임 루프 3파트(무역·항해·전투) + 조선소·술집 · **후반 4종**(거점·공업력 승급·정박·**가공장** — 원료를 사 완제품으로 파는 `state.works`). **끝이 둘** — 조선의 「여덟 항구와 한 척」(`ENDING`)과 아홉 바다 **패권**(`HEGEMONY`). 아홉 바다는 **자동 완주로 검증됐다**(갈래 다섯 1,325항구 · 패권 4/4 — `UNIMPLEMENTED.md` A-8).
...
- **`story/`에 이 세계를 무대로 한 장편소설이 있다.** **게임 데이터를 바꾸지 않는 것이 규약**이고,
  ...
  그 사양이 게임에 전부 들어왔다(해소 표시는 `UNIMPLEMENTED.md`가 정본).
  → `story/GAME-LINK.md` §8 · `story/level/VERDICT.md` §3-2
- **원고는 1장 하나뿐**(`story/ch01-…md`). 순서 정본은 `OUTLINE.md` §집필 순서 — **6부 68장**이고
  **각 부 결말 일곱 장(1·13·33·41·54·62·68)을 먼저** 쓰면 그 일곱만으로 축약본(약 4만 2천 자)이 된다.

## 다음 후보

**목록의 정본은 `UNIMPLEMENTED.md`다**(상태·순서·✅해소 진단까지). 여기는 **방향과 이어받을 자리**만.

- **★ 이어받을 자리 — `UNIMPLEMENTED.md` §손대는 순서.** 완주 테스트 ✅(1,325항구 · 동아시아 패권 4/4) · **NPC 동료 배선**이 1순위(데이터 51명은 있다).
