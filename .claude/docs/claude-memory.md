...
- 게임 루프 3파트(무역·항해·전투) + 조선소·술집 · **후반 4종**(거점·공업력 승급·정박·**가공장** — 원료를 사 완제품으로 파는 `state.works`). **끝이 둘** — 조선의 「여덟 항구와 한 척」(`ENDING`)과 아홉 바다 **패권**(`HEGEMONY`). 아홉 바다는 **손으로** 실클릭 검증했고 **자동 완주 회차는 아직 없다**(`UNIMPLEMENTED.md` A-8).
...
- **★ 이어받을 자리 — 자동개선 회차가 중간에 멈춰 있다**(선단·배속·패권은 들어왔고 **동료·테스트가 남았다**). 절차·미완 목록 정본 → `.playtest/auto-improve/HANDOFF.md`
