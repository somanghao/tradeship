...
- **★ 명부를 닫으면 그 배를 *바다에서도* 내려야 한다.** `pickDef`는 **새로 만들 때만** 닫힌 명부를 후보에서 빼고 **이미 떠 있는 배**는 그대로 둔다 — 그래서 920닢을 치르고 초무한 자가 **39일 뒤 항로를 막았고** `tamed`와 `slain`이 둘 다 박혔다(supremacy ISSUES #26). **돈을 치르고 산 약속이 안 지켜지면 초무라는 길 자체를 아무도 안 산다.** 지금은 `world.js: retireRosterShip()`이 그 이름의 배를 지우고, `state.js`가 **후크**(`setRetireHook` — 모듈 방향을 안 깨려고)로 그것을 부른다. 정원은 안 줄인다(빈자리는 얼굴 없는 배가 채운다). 옛 세이브를 위해 `scenes/map.js`의 조우 갈래가 한 번 더 거른다.
