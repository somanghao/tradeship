...
| **명부 사냥의 값**(사다리인가 벌금인가) · 소식값 · 현상금 실수령 · 사냥터가 값나가는 항로인가 | [wiki/world-npc.md](wiki/world-npc.md) §명부 사냥 | 관측 `node tools/sim-roster.mjs` · 값 `js/data.js: ROSTER`(tipRate·bountyFloorRate·tipFloorAbs) · 상태 `state.bountyDue` |
...
- **계약(대형 주문)의 규모·선금·위약금 함정은 [wiki/contracts.md](wiki/contracts.md)로 옮겼다** —
  계약 규칙을 만질 때만 필요하다(★ 규모는 **도시 규모가 아니라 플레이어 선복**으로 재고,
  위약금은 선금보다 커야 하며, 선금 담보의 하한이 실제로 서는지 확인한다).
- **시뮬로 재는 법과 그 함정은 [wiki/sim-measurement.md](wiki/sim-measurement.md)** — 여러 시드 평균 ·
  `test-world`의 확률 판정 · NPC 압력 반영률.
...
- **★ 사냥터는 이미 값나가는 항로다 — 「순찰비」로 세지 마라.** 명부 40명의 `hunt` 구간을 무역으로 뛰면 **적자가 0/40**이고 중앙이 기준선의 1.16배다. 당연하다 — **해적은 털 것이 지나가는 곳에 앉아 있다.** 실플레이의 −301닢은 **빈 배로 순찰했기 때문**이지 그 구간이 나빠서가 아니었고, 서해 파산도 미조우가 아니라 **20일을 빈 배로 다닌 것**이 원인이었다. ⇒ 사냥이 적자로 보이면 **현상금을 올리기 전에 「그 구간을 무역하며 돌면 얼마인가」를 재라**(`node tools/sim-roster.mjs`의 「빈배로 순찰」 ↔ 「무역하며 순찰」). 게다가 짐을 실으면 `cargoLure`로 **조우가 오른다** — 순찰하며 무역하는 것이 사냥을 **돕는다**.

- **★ `huntedOnLeg`는 `riskKey`가 `def.hunt`와 *정확히 일치*해야 열린다 — 그래서 항로 카드 표시가 곧 성공률이다.** 어느 줄이 그 구간인지 화면이 말해 주지 않으면 그 규칙은 사실상 없는 것과 같다. `map.js`의 항로 카드가 「사냥터」 표를 달고 툴팁이 그 이유를 적는다(P6-5 — 규칙은 한 줄도 안 바뀐다).

- **현상금은 `capSpoils` 밖이다**(P6-2). `capLoot`의 정당화는 *"갑판에 여섯이 금고를 통째로 못 옮긴다"* — **옮겨 싣는 것**이라 상한이 있다. 현상금은 옮겨 싣는 물건이 아니라 **나포심판 뒤 항구에서 관이 치른 돈**이라 갑판 인원과 무관하다. ⇒ `e.bounty`로 따로 들려 `state.bountyDue`에 적고 **다음 입항에 지급**한다. ⚠️ 액수는 안 바꿨다 — 상한만 뗐고, 대신 **살아 돌아와야 받는다**. 실측 중반 세기 5 **+181%**(가난할 때 가장 크게 갈린다).
