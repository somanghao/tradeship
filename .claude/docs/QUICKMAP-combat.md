...
| 어느 항구에서 뭘 짓나 | **사다리 둘을 다 넘어야 한다** — 기술 `industry ≥ tierNeeded` **와** 교역 `industry ≥ yardReach`(0=제 고장·제 나라·이웃 조선지 / 1=같은 바다 / 2=이 항구가 원양으로 직접 잇는 바다 / 3=이 바다가 닿는 바다 / 4=먼 바다). 판정 `state.js: sellsShip/tierNeeded/yardReach/yardCapable/yardShortOf`, 값 `shipPriceAt` |
...
| **도주 성공률** | `state.js: fleeOdds()`가 **정본** — 거리·내 속력 −**적 배 속력**(`SHIPS[enemy.hull].speed`)·돛 증축·양쪽 돛 손상. 0.05~0.95로 묶인다. 말로 옮기는 것은 `fleeWord()` |
| 조우 시 보여 줄 것 | `map.js: foeVersusLine()` — 상대·내 배 수치를 **나란히** + 도주 가망. 전투 도주 단추(`battle.js`)와 **같은 `fleeOdds`를 본다**(어긋나면 안내가 거짓말이 된다) |
...
- **★ 사람은 이길 수 있는 상대만 싸우고 나머지는 피한다 — 설계·측정의 전제다.** 그래서 ① 조우 안내가 **고르기 전에** 상대·내 배 수치를 나란히 보여 주고 도주 가망을 말해 준다(`foeVersusLine`) ② 도주율에 **적 배 속력**이 들어간다(노를 젓는 갤리가 가장 안 놓아준다 — 시작배 기준 50%, 캐랙 62%) ③ 무역 시뮬(`sim-core.mjs`)은 **전리품을 0으로 계산한다** — 회피하는 플레이어가 기준이라 전투 수입을 곡선에 넣지 않는다. 적을 약화시키기 전에 **회피가 성립하는지**부터 본다(시작배도 두 번 안에 66~79%). 표는 `wiki/playtest-log.md` §4-7.
- **`FOES`를 안 적은 바다는 지중해 얼굴이 된다.** 수치는 `ENEMIES` 등급 그대로 쓰고 이름·국적·선체·**깃발**만 갈아 끼우는 구조라, 권역 `npc-pirates.js`에 `FOES`가 없으면 조용히 지중해 적이 나온다(`flag`를 빠뜨리면 명 수군이 부르봉 백합기를 단다).
...
- **★ 「그 바다 배가 없다」는 대개 *순서* 문제다 — 후보에서 빠지는지부터 확인하라.** 나가사키 조선소에서 *"살 수 있는 19척 중 동아시아 배가 한 척도 없다"*는 보고가 나왔는데(supremacy ISSUES #5), 실제로는 **64척이 살 수 있었고 그중 28척이 동아시아 배**였다 — 목록이 `Object.entries(SHIPS)` 순서(= 권역 파일이 합쳐진 순서)라 **이 바다 첫 배가 54번째 줄**에 있었을 뿐이다. 지금은 `shipyard.js: shipOrder()`가 **가진 배 → 이 바다 배(`SHIPS[].home`) → 살 수 있는 배 → 나머지**로 정렬한다(2번째 줄로 올라온다). ⇒ 진단은 `sellsShip`/`yardCapable`에 직접 물어보고 고칠 자리를 정한다. **단순 UX가 아니다** — 권역 패권 조건 ④가 *"그 권역 최고 tier 배를 몬다"*(`hegemonyOf: topShip`)라, 그 배가 안 보이면 완주 조건이 화면에서 막힌다.
- **조선소는 하드코딩된 목록이 아니라 도시 공업력이다.** 예전에는 `SHIPS[].yards`에 판매 항구를 박아 뒀는데, 도시를 늘릴 때마다 어긋나고 "왜 여기선 못 사나"가 설명되지 않았다. 지금은 `industry ≥ tier`로 풀고 `yards`는 **값이 싸지는 전통 조선지 + 교역권 0걸음**이라는 두 뜻을 갖는다 — "거기서만 판다"는 옛 뜻으로 읽지 말 것.
- **★ 공업력이 사다리 하나가 아니라 둘이다 — 기술과 교역.** `sellsShip`이 공업력만 보던 시절에는 공업력 1이면 94종 중 **47종**이 한꺼번에 걸렸고 카라벨·코카 등 **10종이 240곳**에서 지어졌다(나가사키에서 브라질 카라벨랑). 사용자 지시로 `yardReach`를 나란히 놓았고, **좁힌 것이 아니라 계단으로 만든 것**이다(1→6종 · 2→12 · 3→36 · **4→93**). 값을 새로 만들지 않고 `yards`·`originFlag`·`home`·`ROUTES`·`OCEAN_LANES`로만 잰다.
  - ⚠️ **문이 셋이라 셋 다 같은 판정을 지나야 한다** — 신조(`sellsShip`)·중고(`usedListings`, 한 걸음 관대)·나포(`regionPrize`, 원래 권역 한정). 하나만 막으면 나머지로 샌다(`yardsOnly`에서 이미 겪었다).
  - ⚠️ **막다른 길을 만들면 안 된다.** 어느 시작 자리에서도 「여기 또는 항로로 이어진 이웃」에 낡은 바사보다 나은 배가 있어야 한다 — `node tools/sim-firstship.mjs`와 `test-rules.mjs`가 각각 잰다.
  - ⚠️ **`tools/sim-core.mjs: ORDER`는 이제 세계에서 뽑는다.** 손으로 적은 유럽 배 열 척이었고, 교역권이 붙자 아시아 시작 항구가 통째로 「다음 배 없다」가 됐다 — **게임이 아니라 측정기가 유럽만 알고 있었다.** 같은 회차에 그 목록이 넓어지며 드러난 시뮬 버그 둘도 함께 고쳤다: `curRank`가 루프 안에서 안 갱신돼 **작은 배로 하향 갈아타고 있었고**, 매입 판정이 배값만 봐 **선원 34명짜리 배를 사고 파산**했다.
