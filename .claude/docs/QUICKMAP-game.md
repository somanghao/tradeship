...
| **부관 · 부선장 · 에이미** · 고용/해고 · 성과급 · 입항세 감면 · 시장 압력 감면 · 인물 · 리알토 상관 | [wiki/officer.md](wiki/officer.md) | 부관은 **오직 한 명**. 정본 `data.js: OFFICER`. 능력과 성과급은 **한 쌍** — 한쪽만 고치면 균형이 깨진다 |
...
| 부관 급여·성과급·능력·계약금 | `data.js: OFFICER` — `wage`·`cut`·`perks`는 **한 묶음**이라 함께 재측정한다. 급여는 `state.js: voyageCost()`의 `officer` 항목으로 나간다 → [officer.md](wiki/officer.md) |
...
- **부관 능력을 강화하려면 보수도 함께 올린다.** `perks`만 후하게 하면 "고용 안 할 이유가 없는" 장치가 되고, `cut`만 올리면 아무도 안 쓴다. 노림수는 승률 6할.
- **급여(`wage`)와 성과급(`cut`)은 총액이 같아도 효과가 다르다.** 성과급은 잘 벌 때 더 떼므로 성장기 재투자 자본을 깎아 **복리로** 아프고, 급여는 고정이라 규모가 커질수록 가벼워진다. 실제로 총 부담을 맞춰 성과급 12%→8%+급여16으로 갈랐더니 순효과가 +14%→**+21%로 올라갔다.** "총액을 맞췄으니 균형도 같겠지"로 넘기지 말 것.
