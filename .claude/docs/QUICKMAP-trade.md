...
- **★ 후반 브레이크의 효과를 `sim-stat`·짝지은 끝값으로 판정하지 마라 — 측정기가 흡수한다.** `sim-core`의 최적 플레이가 매 항차 관세를 넣어 마진을 다시 재고 **덜 싣는 쪽으로** 옮겨 가는데, 그러면 `MARKET.impact`가 함께 줄어 칸당 마진이 오른다. 한 항차만 달라져도 다음 항구가 바뀌어 궤적이 통째로 갈린다. 같은 시드로 짝지은 **110쌍에서 60항차 차이 중앙값의 부호가 묶음마다 뒤집혔고**(−10,075 / −68 / +1,903 / +3,788닢), cap 6·ceil 60%로 터무니없이 세게 둬도 −18%였다. ⇒ **브레이크를 끈 궤적 위에서 반사실로** 재라 — 항차마다 자산·누진배수·"매출에서 더 떼는 몫"을 셈하면 잡음이 0이다(표는 → [economy-trade.md](wiki/economy-trade.md) §후반 브레이크).
